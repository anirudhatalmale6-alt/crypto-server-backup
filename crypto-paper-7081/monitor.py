...
class CrashDetector:
    ...
    async def _execute_trade(self, crash_id: int, symbol: str, price: float):
        ...
        if self.db.was_manually_closed(symbol, hours=1):
            ...
            return
        total_max = (self.max_simultaneous_trades if self.max_simultaneous_trades > 0 else 0) + (self.max_usdc_trades if self.max_usdc_trades > 0 else 0)
        ...
        if total_max > 0 and len(open_trades) >= total_max:
        ...
        if self.live_mode and self.trader and self.trader.live_mode:
            ...
            allow_eur = self.max_simultaneous_trades != 0 and (self.max_simultaneous_trades < 0 or eur_count < self.max_simultaneous_trades)
            allow_usdc = self.max_usdc_trades != 0 and (self.max_usdc_trades < 0 or usdc_count < self.max_usdc_trades)
    ...
    def _forward_to_1111(self, symbol, price):
        try:
            ...
            resp = _req.post(
                ...
                json={"symbol": symbol, "volume_ratio": 3.0, "ob_ratio": 1.5, "price": price, "source_port": 7081},
