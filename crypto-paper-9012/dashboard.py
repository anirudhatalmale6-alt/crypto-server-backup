        ...
        async function refresh() {
            try {
                ...
                if (data.pnl_today) {
                    ...
                    ptEl.className = 'value ' + (ptVal >= 0 ? 'positive' : 'negative');
                }
...
_binance_cache_8094 = {"portfolio_ext": None, "ts": 0, "open_books": {}}
_proxy_cache = {"usdc_balance": 0, "binance_total_eur": 0, "binance_holdings": [], "ts": 0}
import json as _json_cond
_COND_FILE = "/opt/crypto-paper-9012/cond_close.json"
...
def _save_cond_state():
    try:
        with open(_COND_FILE, "w") as f:
            _json_cond.dump(_cond_state_8094, f)
    except Exception:
        pass

_cond_state_8094 = _load_cond_state()
...
def _get_cached_ticker_symbols(rest_url, quote="USDC", top_n=300, offset=0):
...
def create_app(config: dict, database: Database, detector_ref: list) -> Flask:
    ...
    def api_data():
        ...
        global _binance_cache_8094
        _now = _time.time()
        _cache_ttl = 60
        if _now - _binance_cache_8094["ts"] > _cache_ttl:
            ...
            if open_trades:
                ...
                for t in open_trades:
                    ...
                    if _bp and _bp.get("bid", 0) > 0:
                        ...
                        _new_books[t["symbol"]] = _binance_cache_8094.get("open_books", {}).get(t["symbol"], cp)
            ...
            _binance_cache_8094["portfolio_ext"] = _pext
            _binance_cache_8094["open_books"] = _new_books
            _binance_cache_8094["ts"] = _now

        portfolio_ext = _binance_cache_8094.get("portfolio_ext") or {"open_count": len(open_trades), "total_invested": 0, "current_value": 0, "total_pnl": 0, "pnl_book": 0}
        portfolio_ext["open_count"] = len(open_trades)
        for t in open_trades:
            t["book_price"] = _binance_cache_8094.get("open_books", {}).get(t["symbol"], t.get("current_price", t["entry_price"]))
        ...
        try:
            ...
            _total_usdt = _open_value if _open_value > 0 else 1
        ...
        global _cond_state_8094
        if _cond_state_8094["active"] and resp_data.get("portfolio_ext"):
            _pe = resp_data["portfolio_ext"]
            _book = _pe.get("pnl_book", 0)
            _oc = _pe.get("open_count", 0)
            if _cond_state_8094["cooldown"]:
                if _oc > 0:
                    _cond_state_8094["cooldown"] = False
                    _cond_state_8094["status"] = f"Watching. TP>={_cond_state_8094['tp']:.2f}" if _cond_state_8094["tp"] > 0 else "Watching."
                    if _cond_state_8094["sl"] > 0:
                        _cond_state_8094["status"] += f" SL<=-{_cond_state_8094['sl']:.2f}"
                    _cond_state_8094["status_color"] = "#f0883e"
            elif _oc > 0:
                _triggered = False
                _reason = ""
                if _cond_state_8094["tp"] > 0 and _book >= _cond_state_8094["tp"]:
                    _triggered = True
                    _reason = f"TP TRIGGERED! Book={_book:.2f} USDC."
                    _cond_state_8094["status_color"] = "#3fb950"
                if _cond_state_8094["sl"] > 0 and _book <= -_cond_state_8094["sl"]:
                    _triggered = True
                    _reason = f"SL TRIGGERED! Book={_book:.2f} USDC."
                    _cond_state_8094["status_color"] = "#f85149"
                if _triggered:
                    try:
                        ...
                        pass
                    if _cond_state_8094["mode"] == "once":
                        _cond_state_8094["active"] = False
                        _cond_state_8094["mode"] = "off"
                        _cond_state_8094["cooldown"] = False
                        _cond_state_8094["status"] = _reason + " Closed. (One time - deactivated)"
                        _save_cond_state()
                    else:
                        _cond_state_8094["cooldown"] = True
                        if _cond_state_8094["mode"] == "random" and _cond_state_8094["tp_base"] > 0:
                            _pct = 1 + (_random.random() * 9 + 1) / 100
                            _cond_state_8094["tp"] = round(_cond_state_8094["tp_base"] * _pct, 4)
                            _cond_state_8094["status"] = _reason + f" Closed. Next TP={_cond_state_8094['tp']:.2f}. Waiting..."
                        else:
                            _cond_state_8094["status"] = _reason + " Closed. Waiting for next round..."
                            _save_cond_state()

        resp_data["_v"] = 2
        resp_data["cond_state"] = {
            "active": _cond_state_8094["active"],
            "mode": _cond_state_8094["mode"],
            "tp": _cond_state_8094["tp"],
            "tp_base": _cond_state_8094["tp_base"],
            "sl": _cond_state_8094["sl"],
            "cooldown": _cond_state_8094["cooldown"],
            "status": _cond_state_8094["status"],
            "status_color": _cond_state_8094["status_color"]
        }
    ...
    def get_cond_state():
        global _cond_state_8094
        return jsonify(_cond_state_8094)
    ...
    def set_cond_state():
        ...
        if data.get("action") == "activate":
            _cond_state_8094["active"] = True
            _cond_state_8094["mode"] = data.get("mode", "once")
            _cond_state_8094["tp"] = float(data.get("tp", 0))
            _cond_state_8094["tp_base"] = float(data.get("tp", 0))
            _cond_state_8094["sl"] = float(data.get("sl", 0))
            _cond_state_8094["cooldown"] = False
            if _cond_state_8094["mode"] == "random" and _cond_state_8094["tp_base"] > 0:
                _pct = 1 + (_random.random() * 9 + 1) / 100
                _cond_state_8094["tp"] = round(_cond_state_8094["tp_base"] * _pct, 4)
            label = {"once": "ONE TIME", "always": "ALWAYS", "random": "ALWAYS +RANDOM"}.get(_cond_state_8094["mode"], _cond_state_8094["mode"])
            msg = f"{label}: "
            if _cond_state_8094["tp"] > 0:
                msg += f"TP >= {_cond_state_8094['tp']:.2f} USDC"
                if _cond_state_8094["mode"] == "random":
                    msg += f" (base {_cond_state_8094['tp_base']:.2f} +1-10%)"
                msg += " "
            if _cond_state_8094["sl"] > 0:
                msg += f"SL <= -{_cond_state_8094['sl']:.2f} USDC"
            _cond_state_8094["status"] = msg
            _cond_state_8094["status_color"] = "#f0883e"
            _save_cond_state()
            return jsonify({"success": True, "state": _cond_state_8094})
        elif data.get("action") == "deactivate":
            _cond_state_8094 = {"active": False, "mode": "off", "tp": 0, "tp_base": 0, "sl": 0, "cooldown": False, "status": "Deactivated", "status_color": "#8b949e"}
            return jsonify({"success": True, "state": _cond_state_8094})
    ...
    def history_scan():
        ...
        try:
            ...
            quote = config.get("binance", {}).get("quote_asset", "USDC")
    ...
    def save_settings():
        ...
        try:
            ...
            if "max_simultaneous_trades" in data:
                val = int(data["max_simultaneous_trades"] or 300)
