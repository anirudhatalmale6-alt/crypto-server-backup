...
fetchOverview();
    ...
    def api_receive_signal():
        """Receive trade signal from external port."""
        try:
            data = request.get_json()
            if not data:
            ...
            ob_ratio = float(data.get("ob_ratio", 0))
            price = float(data.get("price", 0))
            source_port = data.get("source_port", "unknown")
            if not symbol or not price:
                return jsonify({"success": False, "error": "Missing symbol or price"})
            det = detector_ref[0] if detector_ref else None
            if det and hasattr(det, 'strategy_manager'):
                n = det.strategy_manager.dispatch_signal(symbol, volume_ratio, ob_ratio, price)
                logger.info(f"EXTERNAL SIGNAL from port {source_port}: {symbol} vol={volume_ratio:.1f}x ob={ob_ratio:.2f} price={price} -> {n} trades opened")
                return jsonify({"success": True, "trades_opened": n})
            return jsonify({"success": False, "error": "Strategy manager not ready"})
