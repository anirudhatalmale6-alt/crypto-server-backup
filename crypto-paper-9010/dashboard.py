...
<body>
    <div class="header">
        ...
        <div class="status">
            <div id="tradingBadge" style="display:inline-flex;align-items:center;gap:6px;background:#1a2332;border:1px solid #238636;border-radius:16px;padding:4px 12px;margin-right:8px;font-size:12px;font-weight:600;color:#3fb950">
                ...
                <span id="tradingModeText">Loading...</span>
    ...
    <div class="container">
        ...
        <div class="stats-grid" id="statsGrid">
            ...
            <div class="stat-card" style="border-color:#f0c000">
                <div class="label" style="color:#f0c000">BINANCE PORTFOLIO (EUR)</div>
                <div class="value" id="statBinanceEur" style="color:#f0c000;font-size:20px">--</div>
                <div id="statBinanceHoldings" style="font-size:11px;color:#8b949e;margin-top:6px;line-height:1.6"></div>
            </div>
            <div class="stat-card" style="border-color:#00d4aa">
                <div class="label" style="color:#00d4aa">USDC PORTFOLIO</div>
                <div class="value" id="statUsdcPortfolio" style="color:#00d4aa;font-size:20px">--</div>
                <div style="font-size:11px;color:#8b949e;margin-top:4px">Total Binance minus port 7081</div>
            </div>
                <div class="stat-card" style="border-color:#ff6b6b">
                    <div class="label" style="color:#ff6b6b">USDC INVESTED</div>
                    <div class="value" id="statUsdcInvested2" style="color:#ff6b6b;font-size:20px">--</div>
                    <div style="font-size:11px;color:#8b949e;margin-top:4px">Open positions (all ports)</div>
                </div>
                <div class="stat-card" style="border-color:#00ff88">
                    <div class="label" style="color:#00ff88">USDC FREE</div>
                    <div class="value" id="statUsdcFree" style="color:#00ff88;font-size:20px">--</div>
                    <div style="font-size:11px;color:#8b949e;margin-top:4px">Available to trade</div>
                </div>
                <div class="stat-card" style="border-color:#f0c000">
                    <div class="label" style="color:#f0c000">EURO PORTFOLIO</div>
                    <div class="value" id="statEurPortfolio" style="color:#f0c000;font-size:20px">--</div>
                    <div style="font-size:11px;color:#8b949e;margin-top:4px">Port 7081 + free EUR</div>
                </div>
                <div class="stat-card" style="border-color:#ff6b6b">
                    <div class="label" style="color:#ff6b6b">EURO INVESTED</div>
                    <div class="value" id="statEurInvested2" style="color:#ff6b6b;font-size:20px">--</div>
                    <div style="font-size:11px;color:#8b949e;margin-top:4px">Port 7081 CONVERT positions</div>
                </div>
                <div class="stat-card" style="border-color:#00ff88">
                    <div class="label" style="color:#00ff88">EURO FREE</div>
                    <div class="value" id="statEurFree" style="color:#00ff88;font-size:20px">--</div>
                    <div style="font-size:11px;color:#8b949e;margin-top:4px">Free EUR on Binance</div>
                </div>
                <div class="stat-card" style="border-color:#f0883e">
                    <div class="label" style="color:#f0883e">PNL Today</div>
                    <div class="value" id="statPnlToday" style="font-size:20px">--</div>
                </div>
            </div>
    ...
    <script>
        ...
        function formatEur(n) {
            if (n === null || n === undefined) return '--';
            let prefix = n >= 0 ? '+' : '';
            return prefix + Number(n).toFixed(2) + ' USDC';
        }
        ...
        async function refresh() {
            try {
                ...
                document.getElementById('statInvested').textContent =
                    formatNumber(data.portfolio.total_invested) + ' USDC';
                ...
                // Binance Portfolio EUR
                if (data.binance_total_eur) {
                    document.getElementById("statBinanceEur").textContent = formatNumber(data.binance_total_eur, 2) + " EUR";
                    var hDiv = document.getElementById("statBinanceHoldings");
                    if (data.binance_holdings && data.binance_holdings.length > 0) {
                        var parts = [];
                        for (var hi = 0; hi < data.binance_holdings.length && hi < 5; hi++) {
                            var h = data.binance_holdings[hi];
                            parts.push(h.asset + ": " + formatNumber(h.eur, 0) + " EUR");
                        }
                        hDiv.textContent = parts.join(" | ");
                    }
                }
                // USDC Portfolio
                if (data.usdc_portfolio) {
                    document.getElementById("statUsdcPortfolio").textContent = formatNumber(data.usdc_portfolio, 2) + " USDC";
                }
                if (data.usdc_invested !== undefined) {
                    document.getElementById("statUsdcInvested2").textContent = formatNumber(data.usdc_invested, 2) + " USDC";
                }
                if (data.usdc_available !== undefined) {
                    document.getElementById("statUsdcFree").textContent = formatNumber(data.usdc_available, 2) + " USDC";
                }
                if (data.eur_invested !== undefined) {
                    var ei = data.eur_invested || 0;
                    var ef = data.eur_free || 0;
                    document.getElementById("statEurPortfolio").textContent = formatNumber(ei + ef, 2) + " EUR";
                    document.getElementById("statEurInvested2").textContent = formatNumber(ei, 2) + " EUR";
                    document.getElementById("statEurFree").textContent = formatNumber(ef, 2) + " EUR";
                }
                ...
                if (data.pnl_today) {
                    ...
                    var ptVal = data.pnl_today.usdt || 0;
                ...
                if (data.open_trades.length > 0) {
                    let html = '<table><tr><th>Time</th><th>Pair</th><th>Entry</th><th>Current</th><th>Amount</th><th>P&L</th><th>Action</th></tr>';
                    data.open_trades.forEach(t => {
                        ...
                        html += `<tr>
                            ...
                            <td>${formatNumber(t.amount_eur)} USDC</td>
                            <td class="${pnlClass}">${formatNumber(t.pnl_percent, 2)}% (${formatEur(t.pnl_eur)})</td>
                            <td><button onclick="closeSinglePosition(${t.id}, '${t.symbol}')" style="background:#e74c3c;color:#fff;border:none;padding:3px 8px;font-size:11px;font-weight:700;cursor:pointer;border-radius:4px">Close</button></td>
                        </tr>`;
                    });
                    html += '</table>';
        ...
        async function closeSinglePosition(tradeId, symbol) {
            if (!confirm('Close ' + symbol + ' at current price?')) return;
            try {
                let r = await fetch('/api/close-position/' + tradeId, {method: 'POST'});
                let d = await r.json();
                if (d.success) { refresh(); } else { alert('Error: ' + (d.error || 'unknown')); }
            } catch(e) { alert('Error: ' + e.message); }
        }

        async function closeAllPositions() {
            if (!confirm('CLOSE ALL - This will close ALL open positions. Are you sure?')) return;
            if (!confirm('FINAL CONFIRMATION: Close ALL positions?')) return;
            try {
                let r = await fetch('/api/close-all', {method: 'POST'});
                let d = await r.json();
                if (d.success) {
                    alert('Closed ' + d.closed + ' positions. Failed: ' + d.failed);
                    refresh();
                } else { alert('Error: ' + (d.error || 'unknown')); }
            } catch(e) { alert('Error: ' + e.message); }
        }

        async function closeAllAuto() {
            try {
                let r = await fetch('/api/close-all', {method: 'POST'});
                let d = await r.json();
...
_binance_cache_9010 = {"portfolio_ext": None, "ts": 0, "open_books": {}}
_proxy_cache = {"usdc_balance": 0, "binance_total_eur": 0, "binance_holdings": [], "ts": 0}
import json as _json_cond
_COND_FILE = "/opt/crypto-paper-9010/cond_close.json"
def _load_cond_state():
    try:
        with open(_COND_FILE, "r") as f:
            return _json_cond.load(f)
    except Exception:
        return {"active": False, "mode": "off", "tp": 0, "tp_base": 0, "sl": 0, "cooldown": False, "status": "Deactivated", "status_color": "#8b949e"}

def _save_cond_state():
    try:
        with open(_COND_FILE, "w") as f:
            _json_cond.dump(_cond_state_9010, f)
    except Exception:
        pass

_cond_state_9010 = _load_cond_state()
...
def create_app(config: dict, database: Database, detector_ref: list) -> Flask:
    ...
    def api_data():
        ...
        # Portfolio ext with 60s cache to avoid Binance WAF
        global _binance_cache_9010
        _now = _time.time()
        _cache_ttl = 60
        if _now - _binance_cache_9010["ts"] > _cache_ttl:
            import requests as _req
            _rest = config.get("binance", {}).get("rest_url", "https://api.binance.com")
            _pext = {"open_count": len(open_trades), "total_invested": 0,
                     "current_value": 0, "total_pnl": 0, "pnl_book": 0}
            _new_books = {}
            if open_trades:
                ...
                for t in open_trades:
                    ...
                    if _bp and _bp.get("bid", 0) > 0:
                        ...
                        _new_books[t["symbol"]] = _binance_cache_9010.get("open_books", {}).get(t["symbol"], cp)
            ...
            _binance_cache_9010["portfolio_ext"] = _pext
            _binance_cache_9010["open_books"] = _new_books
            _binance_cache_9010["ts"] = _now

        portfolio_ext = _binance_cache_9010.get("portfolio_ext") or {"open_count": len(open_trades), "total_invested": 0, "current_value": 0, "total_pnl": 0, "pnl_book": 0}
        portfolio_ext["open_count"] = len(open_trades)
        for t in open_trades:
            t["book_price"] = _binance_cache_9010.get("open_books", {}).get(t["symbol"], t.get("current_price", t["entry_price"]))
        resp_data["portfolio_ext"] = portfolio_ext

        # Proxy Binance data from port 9000 (60s cache)
        global _proxy_cache
        if _now - _proxy_cache["ts"] > 30:
            try:
                import json as _jr
                with open("/opt/crypto-shared/binance_portfolio.json", "r") as _jrf:
                    _jd = _jr.load(_jrf)
                _proxy_cache["usdc_balance"] = _jd.get("usdc_balance", 0)
                _proxy_cache["binance_total_eur"] = _jd.get("binance_total_eur", 0)
                _proxy_cache["binance_holdings"] = _jd.get("binance_holdings", [])
                _proxy_cache["usdc_portfolio"] = _jd.get("usdc_portfolio", 0)
                _proxy_cache["eur_invested"] = _jd.get("eur_invested", 0)
                _proxy_cache["eur_free"] = _jd.get("eur_free", 0)
                _proxy_cache["usdc_invested"] = _jd.get("usdc_invested", 0)
                _proxy_cache["usdc_available"] = _jd.get("usdc_available", 0)
                _proxy_cache["ts"] = _now
            except Exception:
                pass
        resp_data["usdc_balance"] = _proxy_cache.get("usdc_balance", 0)
        resp_data["binance_total_eur"] = _proxy_cache.get("binance_total_eur", 0)
        resp_data["binance_holdings"] = _proxy_cache.get("binance_holdings", [])
        resp_data["usdc_portfolio"] = _proxy_cache.get("usdc_portfolio", 0)
        resp_data["eur_invested"] = _proxy_cache.get("eur_invested", 0)
        resp_data["eur_free"] = _proxy_cache.get("eur_free", 0)
        resp_data["usdc_invested"] = _proxy_cache.get("usdc_invested", 0)
        resp_data["usdc_available"] = _proxy_cache.get("usdc_available", 0)

        # Total Portfolio
        _usdc_bal = resp_data["usdc_balance"]
        _open_value = sum((t.get("current_price") or t.get("entry_price", 0)) * t.get("quantity", 0) for t in open_trades)
        _total_usdc = _usdc_bal + _open_value
        resp_data["total_portfolio"] = {"usdc": round(_total_usdc, 2)}

        # PNL Today calculation
        try:
            ...
            _total_usdt = _open_value
            _today_str = _dt.date.today().isoformat()
            _today_pnl = sum(
                t.get("pnl_eur", 0) for t in closed_trades
                if (t.get("exit_time_str") or "").startswith(_today_str)
            )
            _pct = round(_today_pnl / _total_usdt * 100, 2) if _total_usdt > 0 else 0
            resp_data["pnl_today"] = {
                "usdt": round(_today_pnl, 2),
                "pct": str(_pct)
            }
        except Exception:
            pass
        ...
        global _cond_state_9010
        if _cond_state_9010["active"] and resp_data.get("portfolio_ext"):
            _pe = resp_data["portfolio_ext"]
            _book = _pe.get("pnl_book", 0)
            _oc = _pe.get("open_count", 0)
            if _cond_state_9010["cooldown"]:
                if _oc > 0:
                    _cond_state_9010["cooldown"] = False
                    _cond_state_9010["status"] = f"Watching. TP>={_cond_state_9010['tp']:.2f}" if _cond_state_9010["tp"] > 0 else "Watching."
                    if _cond_state_9010["sl"] > 0:
                        _cond_state_9010["status"] += f" SL<=-{_cond_state_9010['sl']:.2f}"
                    _cond_state_9010["status_color"] = "#f0883e"
            elif _oc > 0:
                _triggered = False
                _reason = ""
                if _cond_state_9010["tp"] > 0 and _book >= _cond_state_9010["tp"]:
                    _triggered = True
                    _reason = f"TP TRIGGERED! Book={_book:.2f} USDC."
                    _cond_state_9010["status_color"] = "#3fb950"
                if _cond_state_9010["sl"] > 0 and _book <= -_cond_state_9010["sl"]:
                    _triggered = True
                    _reason = f"SL TRIGGERED! Book={_book:.2f} USDC."
                    _cond_state_9010["status_color"] = "#f85149"
                if _triggered:
                    try:
                        ...
                        pass
                    if _cond_state_9010["mode"] == "once":
                        _cond_state_9010["active"] = False
                        _cond_state_9010["mode"] = "off"
                        _cond_state_9010["cooldown"] = False
                        _cond_state_9010["status"] = _reason + " Closed. (One time - deactivated)"
                        _save_cond_state()
                    else:
                        _cond_state_9010["cooldown"] = True
                        if _cond_state_9010["mode"] == "random" and _cond_state_9010["tp_base"] > 0:
                            _pct = 1 + (_random.random() * 9 + 1) / 100
                            _cond_state_9010["tp"] = round(_cond_state_9010["tp_base"] * _pct, 4)
                            _cond_state_9010["status"] = _reason + f" Closed. Next TP={_cond_state_9010['tp']:.2f}. Waiting..."
                        else:
                            _cond_state_9010["status"] = _reason + " Closed. Waiting for next round..."
                            _save_cond_state()

        resp_data["_v"] = 2
        resp_data["cond_state"] = {
            "active": _cond_state_9010["active"],
            "mode": _cond_state_9010["mode"],
            "tp": _cond_state_9010["tp"],
            "tp_base": _cond_state_9010["tp_base"],
            "sl": _cond_state_9010["sl"],
            "cooldown": _cond_state_9010["cooldown"],
            "status": _cond_state_9010["status"],
            "status_color": _cond_state_9010["status_color"]
        }
    ...
    def get_cond_state():
        global _cond_state_9010
        return jsonify(_cond_state_9010)
    ...
    def set_cond_state():
        ...
        if data.get("action") == "activate":
            _cond_state_9010["active"] = True
            _cond_state_9010["mode"] = data.get("mode", "once")
            _cond_state_9010["tp"] = float(data.get("tp", 0))
            _cond_state_9010["tp_base"] = float(data.get("tp", 0))
            _cond_state_9010["sl"] = float(data.get("sl", 0))
            _cond_state_9010["cooldown"] = False
            if _cond_state_9010["mode"] == "random" and _cond_state_9010["tp_base"] > 0:
                _pct = 1 + (_random.random() * 9 + 1) / 100
                _cond_state_9010["tp"] = round(_cond_state_9010["tp_base"] * _pct, 4)
            label = {"once": "ONE TIME", "always": "ALWAYS", "random": "ALWAYS +RANDOM"}.get(_cond_state_9010["mode"], _cond_state_9010["mode"])
            msg = f"{label}: "
            if _cond_state_9010["tp"] > 0:
                msg += f"TP >= {_cond_state_9010['tp']:.2f} USDC"
                if _cond_state_9010["mode"] == "random":
                    msg += f" (base {_cond_state_9010['tp_base']:.2f} +1-10%)"
                msg += " "
            if _cond_state_9010["sl"] > 0:
                msg += f"SL <= -{_cond_state_9010['sl']:.2f} USDC"
            _cond_state_9010["status"] = msg
            _cond_state_9010["status_color"] = "#f0883e"
            _save_cond_state()
            return jsonify({"success": True, "state": _cond_state_9010})
        elif data.get("action") == "deactivate":
            _cond_state_9010 = {"active": False, "mode": "off", "tp": 0, "tp_base": 0, "sl": 0, "cooldown": False, "status": "Deactivated", "status_color": "#8b949e"}
            return jsonify({"success": True, "state": _cond_state_9010})
    ...
    def api_close_position(trade_id):
        ...
        sold_on_binance = False
        if trade.get("is_live") and detector_ref and hasattr(detector_ref[0], "trader") and detector_ref[0].trader:
            _t = detector_ref[0].trader
            _prev = _t.live_mode
            _t.live_mode = True
            result = _t.market_sell(trade["symbol"], trade["quantity"])
            _t.live_mode = _prev
            if result:
                close_price = result["avg_price"]
                sold_on_binance = True
                logger.info(f"MANUAL LIVE SELL: {trade['symbol']} #{trade_id} sold {result['filled_qty']} @ {close_price}")
            else:
                logger.error(f"MANUAL LIVE SELL FAILED: {trade['symbol']} #{trade_id}")
                return jsonify({"success": False, "error": "Binance sell failed"})
        logger.info(f"MANUAL CLOSE: {trade['symbol']} #{trade_id} at {close_price}")
        database.close_trade(trade_id, close_price)
        return jsonify({"success": True, "close_price": close_price, "binance_sell": sold_on_binance})
    ...
    def api_close_all():
        ...
        for t in open_trades:
            try:
                ...
                if t.get("is_live") and detector_ref and hasattr(detector_ref[0], "trader") and detector_ref[0].trader:
                    _t = detector_ref[0].trader
                    _prev = _t.live_mode
                    _t.live_mode = True
                    result = _t.market_sell(t["symbol"], t["quantity"])
                    _t.live_mode = _prev
                    if result:
                        cp = result["avg_price"]
                        binance_sold += 1
                        _received = float(result.get("filled_quote", cp * _qty))
                        logger.info(f"CLOSE ALL LIVE SELL: {t['symbol']} #{t['id']} sold {result['filled_qty']} @ {cp}")
                    else:
                        _received = cp * _qty
                        logger.warning(f"CLOSE ALL FALLBACK: {t['symbol']} #{t['id']} Binance sell failed, DB close at {cp}")
                else:
                    _received = cp * _qty
                logger.info(f"CLOSE ALL: {t['symbol']} #{t['id']} at {cp}")
        ...
        return jsonify({"success": True, "closed": closed, "failed": failed, "binance_sold": binance_sold, "total_pnl": _total_pnl, "total_invested": round(_total_invested, 2), "total_received": round(_total_received, 2), "wins": _wins, "losses": _losses, "best": _best, "worst": _worst, "trades": _results})
