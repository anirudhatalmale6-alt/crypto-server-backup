...
class CrashDetector:
    ...
    def _forward_to_1111(self, symbol, price):
        try:
            ...
            resp = _req.post(
                ...
                json={"symbol": symbol, "volume_ratio": 3.0, "ob_ratio": 1.5, "price": price, "source_port": 9010},
    ...
    async def _sweep_overdue_trades(self):
        """Periodically check ALL open trades for max duration, even if not in WebSocket stream."""
        ...
        while True:
            ...
            try:
                ...
                now = time.time()
                for trade in open_trades:
                    dur_limit = trade.get("max_dur_hours") or self.max_duration_hours
                    if dur_limit <= 0:
                        continue
                    elapsed = (now - trade["entry_time"]) / 3600
                    if elapsed < dur_limit:
                        continue
                    symbol = trade["symbol"]
                    try:
                        r = req.get("https://api.binance.com/api/v3/ticker/price",
                                    params={"symbol": symbol}, timeout=5)
                        price = float(r.json().get("price", 0)) if r.status_code == 200 else 0
                    except Exception:
                        price = trade.get("current_price", trade["entry_price"])
                    if price <= 0:
                        price = trade.get("current_price", trade["entry_price"])
                    pnl = ((price - trade["entry_price"]) / trade["entry_price"]) * 100
                    logger.info(f"SWEEP: closing overdue {symbol} ({elapsed:.1f}h > {dur_limit}h)")
                    await self._close_trade(trade, price, pnl, "MAX DURATION")
            except Exception as e:
    ...
    async def _close_trade(self, trade: dict, current_price: float,
        ...
        if is_live and self.trader:
            ...
            if result:
                self._sell_fail_count.pop(tid, None)
                self.db.close_trade(tid, result["avg_price"])
                ...
                self._sell_fail_count[tid] = self._sell_fail_count.get(tid, 0) + 1
                if self._sell_fail_count[tid] >= 3:
                    logger.warning(f"FORCE CLOSE: {symbol} #{tid} after {self._sell_fail_count[tid]} failed Binance sells - closing in DB at {current_price}")
                    self.db.close_trade(tid, current_price)
                    self._sell_fail_count.pop(tid, None)
                else:
                    logger.error(f"LIVE SELL FAILED for {symbol} #{tid} (attempt {self._sell_fail_count[tid]}/3) - will retry")
                return
        else:
            self.db.close_trade(tid, current_price)
            logger.info(
...
class BinanceMonitor:
    ...
    async def start(self):
        ...
        # Periodic sweep for overdue trades not in WS stream
        sweep_task = asyncio.create_task(self.detector._sweep_overdue_trades())
