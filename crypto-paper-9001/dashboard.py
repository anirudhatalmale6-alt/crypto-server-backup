...
<head>
    ...
    <title>Scalping Pump (Cycle 8094 Filtered)</title>
    <style>
...
<body>
    <div class="header">
        <h1>Scalping Pump <span style="font-size:14px;color:#f0883e;font-weight:400">(Cycle 8094 Filtered)</span></h1>
        <div class="status">
    ...
    <script>
        ...
        async function closeAllPositions() {
            ...
            try {
                ...
                let d = await r.json();
                if (d.success) {
                    if (d.total_pnl !== undefined) { showCloseAllResult(d); } else { alert('Closed ' + d.closed + ' positions. Failed: ' + d.failed); }
                    refresh();
                } else {
                    alert('Error: ' + (d.error || 'unknown'));
                }
        ...
        function showCloseAllResult(d) {
            var pnlColor = d.total_pnl >= 0 ? '#3fb950' : '#f85149';
            ...
            box.style.cssText = 'background:#161b22;border:1px solid #30363d;border-radius:12px;padding:30px;min-width:420px;max-width:650px;color:#c9d1d9;font-family:monospace';
            var h = '<h2 style="margin:0 0 20px;color:#c9d1d9;text-align:center;border-bottom:1px solid #30363d;padding-bottom:15px">CLOSE ALL - RESULT</h2>';
            h += '<div style="text-align:center;margin-bottom:20px"><div style="font-size:36px;font-weight:700;color:'+pnlColor+'">'+pnlSign+d.total_pnl.toFixed(4)+' USDC</div>';
            h += '<div style="font-size:13px;color:#8b949e;margin-top:5px">Realized P&L</div></div>';
            h += '<div style="text-align:center;margin-top:15px"><button style="background:#238636;color:#fff;border:none;padding:10px 30px;font-size:14px;font-weight:700;cursor:pointer;border-radius:6px" onclick="this.parentElement.parentElement.parentElement.remove()">OK</button></div>';
            box.innerHTML = h;
        ...
        async function activateCond(mode) {
            ...
            if (tp <= 0 && sl <= 0) { alert('Set at least one value (TP Book or SL Book)'); return; }
            try {
                let r = await fetch('/api/cond-state', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({action: 'activate', mode: mode, tp: tp, sl: sl}) });
                let d = await r.json();
        ...
        async function deactivateCond() {
            try {
                let r = await fetch('/api/cond-state', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({action: 'deactivate'}) });
                let d = await r.json();
                if (d.success) syncCondUI(d.state);
            } catch(e) { alert('Error: ' + e.message); }
        }
        ...
        function checkConditionalClose(data) { if (data.cond_state) syncCondUI(data.cond_state); }
...
_COND_FILE = "/opt/crypto-paper-9001/cond_close.json"
...
def create_app(config: dict, database: Database, detector_ref: list) -> Flask:
    ...
    def api_data():
        ...
        # Proxy Binance data from port 9000 (60s cache - no direct Binance calls)
        global _proxy_cache
        _now = _time.time()
        if _now - _proxy_cache["ts"] > 60:
            try:
                import requests as _preq
                _pr = _preq.get("http://127.0.0.1:9000/api/data", timeout=5)
                _pd = _pr.json()
                _proxy_cache["usdc_balance"] = _pd.get("usdc_balance", 0)
                _proxy_cache["binance_total_eur"] = _pd.get("binance_total_eur", 0)
                _proxy_cache["binance_holdings"] = _pd.get("binance_holdings", [])
                _proxy_cache["usdc_portfolio"] = _pd.get("usdc_portfolio", 0)
                _proxy_cache["eur_invested"] = _pd.get("eur_invested", 0)
                _proxy_cache["eur_free"] = _pd.get("eur_free", 0)
                _proxy_cache["usdc_invested"] = _pd.get("usdc_invested", 0)
                _proxy_cache["usdc_available"] = _pd.get("usdc_available", 0)
        ...
        # Portfolio ext (from local data)
        _pext = {"open_count": len(open_trades), "total_invested": 0, "current_value": 0, "total_pnl": 0, "pnl_book": 0}
        if open_trades:
        ...
        # CLOSE ALL WHEN - server side check
        global _cond_state
                ...
                if _triggered:
                    ...
                    if _cond_state["mode"] == "once":
                        ...
                        if _cond_state["mode"] == "random" and _cond_state["tp_base"] > 0:
                            _rpct = 1 + (_random.random() * 9 + 1) / 100
                            _cond_state["tp"] = round(_cond_state["tp_base"] * _rpct, 4)
