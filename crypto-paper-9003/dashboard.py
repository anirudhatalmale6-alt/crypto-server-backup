...
<body>
    ...
    <div class="container">
        ...
        <div class="settings-panel" style="border-color:#238636;margin-bottom:16px">
            ...
            <div class="settings-grid" style="grid-template-columns:repeat(6,1fr)">
                ...
                <div><label>Pump Window</label><div style="padding:8px 12px;background:#0d1117;border:1px solid #30363d;border-radius:6px;color:#58a6ff;font-weight:600" id="scalpPumpWindow">600s (10 min)</div></div>
                <div><label>Take Profit</label><div style="padding:8px 12px;background:#0d1117;border:1px solid #30363d;border-radius:6px;color:#3fb950;font-weight:600" id="scalpTP">30%</div></div>
                <div><label>Stop Loss</label><div style="padding:8px 12px;background:#0d1117;border:1px solid #30363d;border-radius:6px;color:#f85149;font-weight:600" id="scalpSL">10%</div></div>
            </div>
            <div class="settings-grid" style="grid-template-columns:repeat(4,1fr);margin-top:8px">
                <div><label>Max Duration</label><div style="padding:8px 12px;background:#0d1117;border:1px solid #30363d;border-radius:6px;color:#f0f6fc;font-weight:600" id="scalpDuration">2 hours</div></div>
                ...
                <div><label>Max Positions</label><div style="padding:8px 12px;background:#0d1117;border:1px solid #30363d;border-radius:6px;color:#f0f6fc;font-weight:600" id="scalpMaxPos">100</div></div>
        ...
        <div class="stats-grid" id="statsGrid">
            ...
            <div class="stat-card" style="border-color:#00d4aa">
                ...
                <div class="stat-card" style="border-color:#ff6b6b">
                    ...
                    <div style="font-size:11px;color:#8b949e;margin-top:4px">Open positions (LIVE ports only)</div>
                </div>
    ...
    <script>
        ...
        function formatEur(n) {
            if (n === null || n === undefined) return '--';
            let prefix = n >= 0 ? '+' : '';
            return prefix + Number(n).toFixed(4) + ' USDC';
        }
        ...
        async function refresh() {
            try {
                ...
                if (data.stats.active_config) {
                    ...
                    var sa = document.getElementById("scalpAmount"); if (sa) sa.textContent = c.amount + " USDC";
                    var stp = document.getElementById("scalpTP"); if (stp) stp.textContent = (c.pump_tp||30) + "%";
                    var ssl = document.getElementById("scalpSL"); if (ssl) ssl.textContent = (c.pump_sl||10) + "%";
                    var smp = document.getElementById("scalpMaxPos"); if (smp) smp.textContent = c.max_trades;
                    var sdur = document.getElementById("scalpDuration"); if (sdur) sdur.textContent = (c.max_duration||2) + " hours";
                    var spw = document.getElementById("scalpPumpWindow"); if (spw) { var pw = c.pump_window||600; spw.textContent = pw + "s (" + Math.round(pw/60) + " min)"; }
                }
...
_binance_cache = {"portfolio_ext": None, "usdc_balance": 0, "ts": 0, "open_books": {}, "fetching": False, "binance_total_eur": 0, "binance_holdings": []}
import json as _json_cond
_COND_FILE = "/opt/crypto-paper-9000/cond_close.json"
...
def _save_cond_state():
    try:
        with open(_COND_FILE, "w") as f:
            _json_cond.dump(_cond_state, f)
    except Exception:
        pass

_cond_state = _load_cond_state()
...
def create_app(config: dict, database: Database, detector_ref: list) -> Flask:
    ...
    def api_data():
        ...
        if detector_ref:
            ...
            stats["active_config"] = {
                ...
                "pump_tp": detector.pump_tp,
                "pump_sl": detector.pump_sl,
                "max_duration": detector.max_duration_hours if hasattr(detector, "max_duration_hours") else 2.0,
                "pump_window": detector.pump_window_sec
            }
        ...
        if _now - _binance_cache["ts"] > _cache_ttl and not _binance_cache.get("fetching"):
            ...
            # Fetch full Binance portfolio in EUR + USDC
            _total_eur = 0
            _total_usdt = 0
            _holdings = []
            try:
                _trader = detector_ref[0].trader if detector_ref else None
                if _trader and _trader.live_mode:
                    _acc_params = _trader._sign({})
                    _acc_resp = _req.get(f"{_rest}/api/v3/account", params=_acc_params, headers=_trader._headers(), timeout=10)
                    _acc_resp.raise_for_status()
                    _all_bals = _acc_resp.json().get("balances", [])
                    _nonzero = []
                    for _b in _all_bals:
                        _free = float(_b["free"]) + float(_b.get("locked", 0))
                        if _free > 0.0001:
                            _nonzero.append((_b["asset"], _free))
                    _prices_resp = _req.get(f"{_rest}/api/v3/ticker/price", timeout=10)
                    _prices_resp.raise_for_status()
                    _price_map = {p["symbol"]: float(p["price"]) for p in _prices_resp.json()}
                    _eur_usdt = _price_map.get("EURUSDT", 1.13)
                    for _asset, _qty in _nonzero:
                        _val_usdt = 0
                        if _asset in ("USDT",):
                            _val_usdt = _qty
                        elif _asset in ("USDC",):
                            _val_usdt = _qty * _price_map.get("USDCUSDT", 1.0)
                        elif _asset == "EUR":
                            _val_usdt = _qty * _eur_usdt
                        elif _asset == "BTC":
                            _val_usdt = _qty * _price_map.get("BTCUSDT", 0)
                        elif _asset == "ETH":
                            _val_usdt = _qty * _price_map.get("ETHUSDT", 0)
                        else:
                            _val_usdt = _qty * _price_map.get(f"{_asset}USDT", _price_map.get(f"{_asset}USDC", 0))
                        _val_eur = _val_usdt / _eur_usdt if _eur_usdt > 0 else 0
                        if _val_eur > 0.01:
                            _holdings.append({"asset": _asset, "qty": round(_qty, 4), "eur": round(_val_eur, 2)})
                            _total_eur += _val_eur
                            _total_usdt += _val_usdt
                    _holdings.sort(key=lambda x: x["eur"], reverse=True)
                    try:
                        import sqlite3 as _sq3
                        _db7081 = _sq3.connect("file:///opt/crypto-paper-7081/paper_7081.db?mode=ro", uri=True)
                        _c7081 = _db7081.cursor()
                        _c7081.execute("SELECT symbol, quantity FROM trades WHERE exit_time IS NULL")
                        _p7081_val = sum(_qty7 * _price_map.get(_sym7, 0) for _sym7, _qty7 in _c7081.fetchall())
                        _c7081.execute("SELECT COALESCE(SUM(amount_eur),0) FROM trades WHERE exit_time IS NULL AND is_live=1")
                        _eur_invested = round(_c7081.fetchone()[0], 2)
                        _db7081.close()
                    except Exception:
                        _p7081_val = _binance_cache.get("_port7081_usdt", 0)
                        _eur_invested = _binance_cache.get("eur_invested", 0)
                    _usdc_portfolio = round(_total_usdt - _p7081_val, 2)
                    _eur_free = round(next((h["eur"] for h in _holdings if h["asset"] == "EUR"), 0), 2)
                    _usdc_inv_total = 0
                    for _uport in [9000, 9001, 9002, 9010, 9011, 9012]:
                        try:
                            _udb = _sq3.connect("file:///opt/crypto-paper-%d/paper_%d.db?mode=ro" % (_uport, _uport), uri=True)
                            _uc = _udb.cursor()
                            _uc.execute("SELECT COALESCE(SUM(quantity * entry_price),0) FROM trades WHERE exit_time IS NULL AND is_live=1")
                            _usdc_inv_total += _uc.fetchone()[0]
                            _udb.close()
                        except Exception:
                            pass
                    _usdc_inv_total = round(_usdc_inv_total, 2)
                    _usdc_available = round(_usdc_portfolio - _usdc_inv_total, 2)
            except Exception:
                _total_eur = _binance_cache.get("binance_total_eur", 0)
                _total_usdt = _binance_cache.get("_total_usdt", 0)
                _p7081_val = _binance_cache.get("_port7081_usdt", 0)
                _usdc_portfolio = _binance_cache.get("usdc_portfolio", 0)
                _holdings = _binance_cache.get("binance_holdings", [])
                _eur_invested = _binance_cache.get("eur_invested", 0)
                _eur_free = _binance_cache.get("eur_free", 0)
                _usdc_inv_total = _binance_cache.get("usdc_invested", 0)
                _usdc_available = _binance_cache.get("usdc_available", 0)

            _binance_cache["binance_total_eur"] = round(_total_eur, 2)
            _binance_cache["binance_holdings"] = _holdings
            _binance_cache["usdc_portfolio"] = _usdc_portfolio if "_usdc_portfolio" in dir() else _binance_cache.get("usdc_portfolio", 0)
            _binance_cache["_total_usdt"] = _total_usdt
            _binance_cache["_port7081_usdt"] = _p7081_val if "_p7081_val" in dir() else 0
            _binance_cache["eur_invested"] = _eur_invested if "_eur_invested" in dir() else _binance_cache.get("eur_invested", 0)
            _binance_cache["eur_free"] = _eur_free if "_eur_free" in dir() else _binance_cache.get("eur_free", 0)
            _binance_cache["usdc_invested"] = _usdc_inv_total if "_usdc_inv_total" in dir() else _binance_cache.get("usdc_invested", 0)
            _binance_cache["usdc_available"] = _usdc_available if "_usdc_available" in dir() else _binance_cache.get("usdc_available", 0)
            ...
            try:
                import json as _jw
                _jw_data = {"usdc_balance": _usdc, "binance_total_eur": round(_total_eur, 2), "binance_holdings": _holdings, "usdc_portfolio": _binance_cache.get("usdc_portfolio", 0), "eur_invested": _binance_cache.get("eur_invested", 0), "eur_free": _binance_cache.get("eur_free", 0), "usdc_invested": _binance_cache.get("usdc_invested", 0), "usdc_available": _binance_cache.get("usdc_available", 0), "ts": _now}
                with open("/opt/crypto-shared/binance_portfolio.json", "w") as _jf:
                    _jw.dump(_jw_data, _jf)
            except Exception:
                pass
        ...
        resp_data["binance_total_eur"] = _binance_cache.get("binance_total_eur", 0)
        resp_data["binance_holdings"] = _binance_cache.get("binance_holdings", [])
        resp_data["usdc_portfolio"] = _binance_cache.get("usdc_portfolio", 0)
        resp_data["eur_invested"] = _binance_cache.get("eur_invested", 0)
        resp_data["eur_free"] = _binance_cache.get("eur_free", 0)
        resp_data["usdc_invested"] = _binance_cache.get("usdc_invested", 0)
        resp_data["usdc_available"] = _binance_cache.get("usdc_available", 0)
        ...
        try:
            ...
            for _op in [9010, 9012]:
                try:
                    _or = _req2.get(f"http://127.0.0.1:{_op}/api/data", timeout=3).json()
                    _other_invested += _or.get("portfolio", {}).get("total_invested", 0) or 0
                except Exception:
                    pass
            total_usdc = usdc_bal + open_value + _other_invested
            resp_data["other_invested"] = round(_other_invested, 2)
    ...
    def api_close_position(trade_id):
        ...
        _trader = detector_ref[0].trader if detector_ref else None
        _is_live_trade = trade.get("is_live", False)
        if _trader and (_trader.live_mode or _is_live_trade):
            if _is_live_trade and not _trader.live_mode:
                _trader.live_mode = True
                _force_restore = True
            else:
