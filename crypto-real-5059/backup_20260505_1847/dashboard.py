        ...
        async function applyPaperTrading() {
            ...
            try {
                let payload = {
                    ...
                    pump_detection: document.getElementById('btPumpEnabled').value,
...
def create_app(config: dict, database: Database, detector_ref: list) -> Flask:
    ...
    def index():
        ...
        return render_template_string(
            ...
            pump_only_mode=(detection_cfg.get("pump_detection") == "pump_only"),
    ...
    def _run_hybrid_backtest(req, rest_url, quote, start_ts, end_ts,
        ...
        def hybrid_backtest_symbol(symbol):
            trades = []
            try:
                # INSTANT PUMP: scan 1-minute klines for rapid price rises
                if crash_threshold >= 99999:
                    klines_1m = fetch_klines(symbol, "1m", start_ts, end_ts, max_pages=100)
                    if not klines_1m:
                        return trades
                    wc = max(1, pump_window_sec // 60) if pump_window_sec >= 60 else 1
                    _in_t = False
                    _ep = 0
                    _et = 0
                    _ed = ""
                    for i in range(len(klines_1m)):
                        _ts = int(klines_1m[i][0])
                        _hi = float(klines_1m[i][2])
                        _lo = float(klines_1m[i][3])
                        _cl = float(klines_1m[i][4])
                        _dt = datetime.fromtimestamp(_ts / 1000).strftime("%Y-%m-%d %H:%M:%S")
                        if _in_t:
                            _ph = ((_hi - _ep) / _ep) * 100
                            _pl = ((_lo - _ep) / _ep) * 100
                            if _ph >= pump_tp:
                                _xp = _ep * (1 + pump_tp / 100)
                                trades.append({"symbol": symbol, "entry_date": _ed, "exit_date": _dt, "entry_price": _ep, "exit_price": round(_xp, 8), "pnl_pct": round(pump_tp, 2), "pnl_usdt": round(amount * pump_tp / 100, 2), "exit_reason": "TP", "entry_type": "pump"})
                                _in_t = False
                            elif _pl <= -pump_sl:
                                _xp = _ep * (1 - pump_sl / 100)
                                trades.append({"symbol": symbol, "entry_date": _ed, "exit_date": _dt, "entry_price": _ep, "exit_price": round(_xp, 8), "pnl_pct": round(-pump_sl, 2), "pnl_usdt": round(-amount * pump_sl / 100, 2), "exit_reason": "SL", "entry_type": "pump"})
                                _in_t = False
                            elif max_duration_hours > 0 and (_ts / 1000 - _et) >= max_duration_hours * 3600:
                                _pnl = ((_cl - _ep) / _ep) * 100
                                trades.append({"symbol": symbol, "entry_date": _ed, "exit_date": _dt, "entry_price": _ep, "exit_price": round(_cl, 8), "pnl_pct": round(_pnl, 2), "pnl_usdt": round(amount * _pnl / 100, 2), "exit_reason": "TIME", "entry_type": "pump"})
                                _in_t = False
                        else:
                            _lb = max(0, i - wc)
                            _ml = min(float(klines_1m[j][3]) for j in range(_lb, i + 1))
                            if _ml > 0 and _cl > _ml:
                                _rise = ((_cl - _ml) / _ml) * 100
                                if _rise >= pump_threshold:
                                    _in_t = True
                                    _ep = _cl
                                    _et = _ts / 1000
                                    _ed = _dt
                    if _in_t and klines_1m:
                        _lk = klines_1m[-1]
                        _lc = float(_lk[4])
                        _pnl = ((_lc - _ep) / _ep) * 100
                        trades.append({"symbol": symbol, "entry_date": _ed, "exit_date": datetime.fromtimestamp(int(_lk[0]) / 1000).strftime("%Y-%m-%d %H:%M:%S"), "entry_price": _ep, "exit_price": round(_lc, 8), "pnl_pct": round(_pnl, 2), "pnl_usdt": round(amount * _pnl / 100, 2), "exit_reason": "OPEN", "entry_type": "pump"})
                    return trades
                ...
                for i in range(len(klines)):
                    ...
                    pump_pct = ((close - opn) / opn) * 100 if opn > 0 else 0
                ...
                while sig_idx < len(signal_candles):
                    ...
                    if trade_closed and sig_idx < len(signal_candles):
                        ...
                        while sig_idx < len(signal_candles):
                            next_sig_date = datetime.fromtimestamp(signal_candles[sig_idx]["ts"] / 1000).strftime("%Y-%m-%d %H:%M:%S")
                            if next_sig_date > exit_ts:
                                break
                            sig_idx += 1
    ...
    def save_settings():
        ...
        try:
            ...
            if "pump_detection" in data:
                raw = data["pump_detection"]
                if raw == "pump_only":
                    detection_cfg["pump_detection"] = "pump_only"
                    if detector_ref:
                        detector_ref[0].pump_detection = True
                        detector_ref[0].pump_only_mode = True
                elif raw == "on" or raw is True:
                    detection_cfg["pump_detection"] = True
                    if detector_ref:
                        detector_ref[0].pump_detection = True
                        detector_ref[0].pump_only_mode = False
                else:
