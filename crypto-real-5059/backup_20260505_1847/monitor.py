...
class CrashDetector:
    ...
    def __init__(self, config: dict, database: Database, alert_manager: AlertManager,
        ...
        self.pump_only_mode = (detection_cfg.get("pump_detection") == "pump_only")
        if self.pump_only_mode:
            self.pump_detection = True
        ...
        self.stats = {
            ...
            "pump_only_mode": getattr(self, 'pump_only_mode', False)
        }
    ...
    async def process_price(self, symbol: str, price: float):
        ...
        for window_min in self.time_windows:
            ...
            if drop_percent >= self.drop_threshold:
                if self.reversal_mode != "pump_only" and not self.pump_only_mode:
        ...
        if self.pump_detection or self.reversal_mode == "pump_only" or self.pump_only_mode:
    ...
    async def _handle_crash(self, symbol: str, drop_percent: float,
        ...
        if self.trading_enabled:
            ...
            if self.reversal_mode == "pump_only":
                await self._execute_trade(crash_id, symbol, price_end)
            elif self.reversal_mode == "reversal":
...
class BinanceMonitor:
    ...
    def fetch_top_pairs(self) -> List[str]:
        """Fetch top trading pairs by volume from Binance REST API."""
