    ...
    <div class="container">
        ...
        <div style="display:flex; flex-wrap:wrap; gap:8px; align-items:center; margin-top:8px; padding:10px 16px; background:#161b22; border:1px solid #30363d; border-radius:6px;">
            ...
            <div style="display:flex;align-items:center;gap:4px;">
                <label style="color:#8b949e;font-size:12px;white-space:nowrap;">TP USDC</label>
            ...
            <div style="display:flex;align-items:center;gap:4px;">
                <label style="color:#8b949e;font-size:12px;white-space:nowrap;">TP Book USDC</label>
            ...
            <div style="display:flex;align-items:center;gap:4px;">
                <label style="color:#8b949e;font-size:12px;white-space:nowrap;">SL USDC</label>
            ...
            <div style="display:flex;align-items:center;gap:4px;">
                <label style="color:#8b949e;font-size:12px;white-space:nowrap;">SL Book USDC</label>
    ...
    <script>
        ...
        function updateSelection() {
            ...
            document.getElementById('selSummary').innerHTML =
                ...
                'PnL: <span style="color:' + pc + ';font-weight:700;">' + ps + formatNumber(totalPnl, 2) + ' USDC</span>' + pnlEurStr + ' | ' +
                'BOOK: <span style="color:' + bc + ';font-weight:700;">' + bs + formatNumber(totalBook, 2) + ' USDC</span>' + bookEurStr;
        }
        ...
        async function toggleConditionalSel() {
            ...
            try {
                ...
                if (d.success) {
                    ...
                    document.getElementById('condSelStatus').innerHTML = '<span style="color:#f0883e;">ACTIVE: ' + d.count + ' pos | ' + parts.join(' | ') + ' USDC</span>';
                }
            } catch(e) { alert('Error: ' + e.message); }
        }
        ...
        async function toggleConditionalAll() {
            ...
            try {
                ...
                if (d.success) {
                    ...
                    if (sl > 0) parts.push('SL: -' + sl);
                    if (slBook > 0) parts.push('SL Book: -' + slBook);
                    document.getElementById('condAllStatus').innerHTML = '<span style="color:#f0883e;">ACTIVE: ALL pos | ' + parts.join(' | ') + ' USDC</span>';
                }
            } catch(e) { alert('Error: ' + e.message); }
        }
        ...
        function checkConditionalStatus(data) {
            ...
            data.conditional_triggered.forEach(function(t) {
                if (t.mode === 'sel') {
                    ...
                    document.getElementById('btnCondSelActivate').textContent = 'ACTIVATE';
                    document.getElementById('btnCondSelActivate').style.background = '#238636';
                    document.getElementById('condSelStatus').innerHTML = '<span style="color:#3fb950;">TRIGGERED: Closed ' + t.closed + ' pos (' + t.reason + ': ' + t.value.toFixed(2) + ' USDC)</span>';
                } else {
                    condAllActive = false;
                    document.getElementById('btnCondAllActivate').textContent = 'ACTIVATE';
                    document.getElementById('btnCondAllActivate').style.background = '#238636';
                    document.getElementById('condAllStatus').innerHTML = '<span style="color:#3fb950;">TRIGGERED: Closed ' + t.closed + ' pos (' + t.reason + ': ' + t.value.toFixed(2) + ' USDC)</span>';
                }
            });
            refresh();
        }
            ...
            try {
                ...
                if (data.stats.active_config) {
                    ...
                    document.getElementById('activeConfig').textContent =
                        c.threshold + '% | ' + c.windows.join(',') + 'm | TP:' + c.tp + '% SL:' + c.sl + '% | ' + c.amount + ' USDC | ' + c.coins + ' coins | Max:' + (c.max_trades > 0 ? c.max_trades + ' trades' : 'unlimited');
                }
                ...
                if (data.portfolio_ext) {
                    ...
                    document.getElementById('sumInvested').innerHTML =
                        formatNumber(pe.total_invested, 2) + ' USDC' + eurLine(pe.total_invested, '#c9d1d9');
                ...
                if (data.open_trades.length > 0) {
                    ...
                    let html = '<table><tr><th><input type="checkbox" id="selectAll" onchange="toggleAll(this)"></th><th>Coin</th><th>Qty</th><th>Entry</th><th>Current</th><th>Invested</th><th>Value</th><th>PnL</th><th></th></tr>';
                    data.open_trades.forEach(t => {
                        var coin = t.symbol.replace('USDC','');
                        ...
                        var pnlHtml = '<span style="color:' + pc + ';font-weight:700;">' + ps + formatNumber(pnlPct, 2) + '% (' + ps + formatNumber(pnlUsdt, 2) + ' USDC)</span>';
                        if (er2 > 0) pnlHtml += '<br><span style="color:' + pc + ';">' + ps + formatNumber(pnlEurVal, 2) + ' EUR</span>';
                        pnlHtml += '<br><span style="color:' + bc + ';">BOOK: ' + bs + formatNumber(bookPct, 2) + '% (' + bs + formatNumber(bookPnl, 2) + ' USDC)</span>';
                        ...
                        html += '<tr>' +
                            ...
                            '<td>' + pnlHtml + '</td>' +
                            '<td><button onclick="closePosition(' + t.id + ')" style="background:#e74c3c;color:#fff;border:none;padding:6px 14px;font-size:12px;font-weight:700;cursor:pointer;border-radius:4px;">Close</button></td>' +
                            '</tr>';
                    });
                    html += '</table>';
            ...
            try {
                ...
                if (data.error) {
                    ...
                    summary.innerHTML = '<div class="stats-grid" style="margin-top:8px">' +
                        ...
                        '<div class="stat-card"><div class="label">Total P&L</div><div class="value ' + pnlClass + '">' + formatNumber(s.total_pnl,2) + ' USDC</div></div>' +
                        ...
                        '<div class="stat-card"><div class="label">Max Capital Needed</div><div class="value">' + formatNumber(s.max_capital_needed||0, 0) + ' USDC</div></div>' +
                    ...
                    if (data.trades.length > 0) {
                        let html = '<table><tr><th>#</th><th>Pair</th><th>Type</th><th>Entry Date</th><th>Exit Date</th><th>Duration</th><th>Entry</th><th>Exit</th><th>P&L %</th><th>P&L USDC</th><th>Exit Reason</th></tr>';
        ...
        (async function() {
            try {
                ...
                if (d.sel && d.sel.active) {
                    ...
                    document.getElementById('condSelStatus').innerHTML = '<span style="color:#f0883e;">ACTIVE: ' + d.sel.count + ' pos | ' + parts.join(' | ') + ' USDC</span>';
                }
...
def create_app(config: dict, database: Database, detector_ref: list) -> Flask:
    ...
    def close_selected_positions():
        """Close selected positions using order book price."""
        try:
            data = request.get_json()
            ids = data.get("ids", [])
            if not ids:
            ...
            for trade in open_trades:
                if trade["id"] in ids:
                    ...
                    database.close_trade(trade["id"], price)
                    closed += 1
    ...
    def close_position(trade_id):
        """Close a single position using order book price."""
        try:
            ...
            if price is None:
                price = trade.get("current_price") or trade["entry_price"]
            database.close_trade(trade_id, price)
    ...
    def close_all_positions():
        """Close all open positions using order book price."""
        try:
            open_trades = database.get_open_trades()
            if not open_trades:
