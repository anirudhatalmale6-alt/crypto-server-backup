...
_binance_cache_8093 = {"portfolio_ext": None, "ts": 0, "open_books": {}}
_cond_state_8093 = {"active": False, "mode": "off", "tp": 0, "tp_base": 0, "sl": 0, "cooldown": False, "status": "Deactivated", "status_color": "#8b949e"}
...
def create_app(config: dict, database: Database, detector_ref: list) -> Flask:
    ...
    def api_data():
        ...
        global _binance_cache_8093
        _now = _time.time()
        _cache_ttl = 60
        if _now - _binance_cache_8093["ts"] > _cache_ttl:
            ...
            if open_trades:
                ...
                for t in open_trades:
                    ...
                    if _bp and _bp.get("bid", 0) > 0:
                        ...
                        _new_books[t["symbol"]] = _binance_cache_8093.get("open_books", {}).get(t["symbol"], cp)
            ...
            _binance_cache_8093["portfolio_ext"] = _pext
            _binance_cache_8093["open_books"] = _new_books
            _binance_cache_8093["ts"] = _now

        portfolio_ext = _binance_cache_8093.get("portfolio_ext") or {"open_count": len(open_trades), "total_invested": 0, "current_value": 0, "total_pnl": 0, "pnl_book": 0}
        portfolio_ext["open_count"] = len(open_trades)
        for t in open_trades:
            t["book_price"] = _binance_cache_8093.get("open_books", {}).get(t["symbol"], t.get("current_price", t["entry_price"]))
        ...
        global _cond_state_8093
        if _cond_state_8093["active"] and resp_data.get("portfolio_ext"):
            _pe = resp_data["portfolio_ext"]
            _book = _pe.get("pnl_book", 0)
            _oc = _pe.get("open_count", 0)
            if _cond_state_8093["cooldown"]:
                if _oc > 0:
                    _cond_state_8093["cooldown"] = False
                    _cond_state_8093["status"] = f"Watching. TP>={_cond_state_8093['tp']:.2f}" if _cond_state_8093["tp"] > 0 else "Watching."
                    if _cond_state_8093["sl"] > 0:
                        _cond_state_8093["status"] += f" SL<=-{_cond_state_8093['sl']:.2f}"
                    _cond_state_8093["status_color"] = "#f0883e"
            elif _oc > 0:
                _triggered = False
                _reason = ""
                if _cond_state_8093["tp"] > 0 and _book >= _cond_state_8093["tp"]:
                    _triggered = True
                    _reason = f"TP TRIGGERED! Book={_book:.2f} USDT."
                    _cond_state_8093["status_color"] = "#3fb950"
                if _cond_state_8093["sl"] > 0 and _book <= -_cond_state_8093["sl"]:
                    _triggered = True
                    _reason = f"SL TRIGGERED! Book={_book:.2f} USDT."
                    _cond_state_8093["status_color"] = "#f85149"
                if _triggered:
                    try:
                        ...
                        pass
                    if _cond_state_8093["mode"] == "once":
                        _cond_state_8093["active"] = False
                        _cond_state_8093["mode"] = "off"
                        _cond_state_8093["cooldown"] = False
                        _cond_state_8093["status"] = _reason + " Closed. (One time - deactivated)"
                    else:
                        _cond_state_8093["cooldown"] = True
                        if _cond_state_8093["mode"] == "random" and _cond_state_8093["tp_base"] > 0:
                            _pct = 1 + (_random.random() * 9 + 1) / 100
                            _cond_state_8093["tp"] = round(_cond_state_8093["tp_base"] * _pct, 4)
                            _cond_state_8093["status"] = _reason + f" Closed. Next TP={_cond_state_8093['tp']:.2f}. Waiting..."
                        else:
                            _cond_state_8093["status"] = _reason + " Closed. Waiting for next round..."

        resp_data["_v"] = 2
        resp_data["cond_state"] = {
            "active": _cond_state_8093["active"],
            "mode": _cond_state_8093["mode"],
            "tp": _cond_state_8093["tp"],
            "tp_base": _cond_state_8093["tp_base"],
            "sl": _cond_state_8093["sl"],
            "cooldown": _cond_state_8093["cooldown"],
            "status": _cond_state_8093["status"],
            "status_color": _cond_state_8093["status_color"]
        }
    ...
    def get_cond_state():
        global _cond_state_8093
        return jsonify(_cond_state_8093)
    ...
    def set_cond_state():
        ...
        if data.get("action") == "activate":
            _cond_state_8093["active"] = True
            _cond_state_8093["mode"] = data.get("mode", "once")
            _cond_state_8093["tp"] = float(data.get("tp", 0))
            _cond_state_8093["tp_base"] = float(data.get("tp", 0))
            _cond_state_8093["sl"] = float(data.get("sl", 0))
            _cond_state_8093["cooldown"] = False
            if _cond_state_8093["mode"] == "random" and _cond_state_8093["tp_base"] > 0:
                _pct = 1 + (_random.random() * 9 + 1) / 100
                _cond_state_8093["tp"] = round(_cond_state_8093["tp_base"] * _pct, 4)
            label = {"once": "ONE TIME", "always": "ALWAYS", "random": "ALWAYS +RANDOM"}.get(_cond_state_8093["mode"], _cond_state_8093["mode"])
            msg = f"{label}: "
            if _cond_state_8093["tp"] > 0:
                msg += f"TP >= {_cond_state_8093['tp']:.2f} USDT"
                if _cond_state_8093["mode"] == "random":
                    msg += f" (base {_cond_state_8093['tp_base']:.2f} +1-10%)"
                msg += " "
            if _cond_state_8093["sl"] > 0:
                msg += f"SL <= -{_cond_state_8093['sl']:.2f} USDT"
            _cond_state_8093["status"] = msg
            _cond_state_8093["status_color"] = "#f0883e"
            return jsonify({"success": True, "state": _cond_state_8093})
