...
class CrashDetector:
    ...
    def __init__(self, config: dict, database: Database, alert_manager: AlertManager,
        ...
        self.max_trades_per_day = trading_cfg.get("max_trades_per_day", 0)
        self.pair_mode = trading_cfg.get("pair_mode", "all")
        self._usdc_bases = set()
    ...
    def _load_usdc_bases(self):
        try:
            import requests
            rest_url = self.config.get("binance", {}).get("rest_url", "https://api.binance.com")
            r = requests.get(rest_url + "/api/v3/exchangeInfo", timeout=30)
            info = r.json()
            self._usdc_bases = set()
            for s in info.get("symbols", []):
                if s.get("quoteAsset") == "USDC" and s.get("status") == "TRADING":
                    self._usdc_bases.add(s["baseAsset"])
            logger.info("Loaded %d USDC bases for pair_mode filter" % len(self._usdc_bases))
        except Exception as e:
            logger.error("Failed to load USDC bases: %s" % e)
    ...
    async def _execute_trade(self, crash_id: int, symbol: str, price: float):
        ...
        if self.max_trades_per_day > 0:
            ...
            if self._trades_today_date != today_str:
                self._trades_today_date = today_str
                self._trades_today_count = 0
            if self._trades_today_count >= self.max_trades_per_day:
                logger.info(
                    f"MAX DAILY TRADES REACHED: {self._trades_today_count}/{self.max_trades_per_day} "
                    f"today - skipping {symbol}"
                )
                return
        if self.pair_mode != "all":
            base = symbol.replace("USDT", "")
            if not self._usdc_bases:
                self._load_usdc_bases()
            if self.pair_mode == "usdc" and base not in self._usdc_bases:
                logger.info(f"PAIR MODE USDC: {symbol} has no USDC pair - will use Convert fallback")
            elif self.pair_mode == "convert" and base in self._usdc_bases:
                logger.info(f"PAIR MODE CONVERT: {symbol} has USDC pair - skipping")
                return
        if self.live_mode and self.trader and self.trader.live_mode:
            actual_amount = self.trade_amount
            balance = self.trader.get_account_balance("USDC")
            if balance is not None:
                if balance < 100:
                    logger.warning(f"INSUFFICIENT BALANCE: {balance:.2f} USDC available, minimum 100 USDC needed - skipping {symbol}")
                    return
                if balance < self.trade_amount:
                    logger.info(f"BALANCE ADJUSTED: {balance:.2f} USDC available (configured {self.trade_amount}), trading with {balance:.2f}")
                    actual_amount = balance
            result = self.trader.market_buy(symbol, actual_amount)
            if result:
    ...
    def _forward_to_1111(self, symbol, price):
        try:
            ...
            resp = _req.post(
                ...
                json={"symbol": symbol, "volume_ratio": 3.0, "ob_ratio": 1.5, "price": price, "source_port": 5057},
