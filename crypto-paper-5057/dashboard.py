...
<body>
    ...
    <div class="container">
            ...
            <div id="settingsBody">
                <div class="settings-grid">
                    ...
                    <div class="setting-item">
                        ...
                        <select id="setMaxPerDay">
                            <option value="0" {{ 'selected' if max_trades_per_day == 0 else '' }}>Unlimited</option>
                            <option value="1" {{ 'selected' if max_trades_per_day == 1 else '' }}>1</option>
                            <option value="2" {{ 'selected' if max_trades_per_day == 2 else '' }}>2</option>
                            <option value="3" {{ 'selected' if max_trades_per_day == 3 else '' }}>3</option>
                            <option value="5" {{ 'selected' if max_trades_per_day == 5 else '' }}>5</option>
                            <option value="10" {{ 'selected' if max_trades_per_day == 10 else '' }}>10</option>
                        </select>
                    </div>
                    <div class="setting-item">
                        <label>Pair Mode</label>
                        <select id="setPairMode">
                            <option value="all" {{ 'selected' if pair_mode == 'all' else '' }}>USDC + Convert</option>
                            <option value="usdc" {{ 'selected' if pair_mode == 'usdc' else '' }}>USDC Only</option>
                            <option value="convert" {{ 'selected' if pair_mode == 'convert' else '' }}>Convert Only</option>
                        </select>
                    </div>
                </div>
        ...
        <div class="settings-panel">
            ...
            <div id="backtestBody" style="display:none">
                ...
                <div class="settings-grid">
                    ...
                    <div class="setting-item">
                        <label>Pair Mode</label>
                        <select id="btPairMode">
                            <option value="all" selected>USDC + Convert</option>
        ...
        <div class="section">
            <div class="section-header">
                <span>Recent Pumps (alerts triggered)</span>
                <span class="badge" id="pumpCount">0</span>
            </div>
            <div id="pumpTable">
                <div class="empty-state">No pumps detected yet. Monitoring (5% in 5s)...</div>
            </div>
        </div>
    ...
    <script>
            ...
            try {
                ...
                if (data.top_drops && data.top_drops.length > 0) {
                    ...
                    data.top_drops.forEach((d, i) => {
                        ...
                        let asset_d = d.symbol.replace('USDT','').replace('USDC','');
                        html += `<tr>
                            <td>${i+1}</td>
                            <td><a href="https://www.binance.com/en/trade/${asset_d}_USDT" target="_blank" style="color:#58a6ff;text-decoration:none;"><strong>${d.symbol}</strong></a></td>
                            <td class="${dropClass}">-${formatNumber(d.drop, 2)}%</td>
                            <td><a href="https://www.tradingview.com/chart/?symbol=BINANCE:${d.symbol}" target="_blank" style="color:#58a6ff;text-decoration:underline;">${formatNumber(d.high, 8)}</a></td>
                ...
                if (data.crashes.length > 0) {
                    ...
                    data.crashes.forEach(c => {
                        ...
                        let asset = c.symbol.replace('USDT','').replace('USDC','');
                        html += `<tr ${cls}>
                            <td><a href="https://www.tradingview.com/chart/?symbol=BINANCE:${c.symbol}" target="_blank" style="color:#58a6ff;text-decoration:underline;">${c.detected_at_str}</a></td>
                            <td><a href="https://www.binance.com/en/trade/${asset}_USDT" target="_blank" style="color:#58a6ff;text-decoration:none;"><strong>${c.symbol}</strong></a></td>
                ...
                // Pumps table
                let pumpDiv = document.getElementById('pumpTable');
                document.getElementById('pumpCount').textContent = data.pumps ? data.pumps.length : 0;
                if (data.pumps && data.pumps.length > 0) {
                    let html = '<table><tr><th>Time</th><th>Pair</th><th>Pump</th><th>From</th><th>To</th><th>Window</th></tr>';
                    data.pumps.forEach(p => {
                        let asset = p.symbol.replace('USDT','').replace('USDC','');
                        html += `<tr>
                            <td><a href="https://www.tradingview.com/chart/?symbol=BINANCE:${p.symbol}" target="_blank" style="color:#58a6ff;text-decoration:underline;">${p.detected_at_str}</a></td>
                            <td><a href="https://www.binance.com/en/trade/${asset}_USDT" target="_blank" style="color:#58a6ff;text-decoration:none;"><strong>${p.symbol}</strong></a></td>
                            <td class="pnl-positive">+${formatNumber(p.drop_percent, 1)}%</td>
                            <td>${formatNumber(p.price_start, 8)}</td>
                            <td>${formatNumber(p.price_end, 8)}</td>
                            <td>${p.time_window_min < 1 ? (p.time_window_min * 60).toFixed(0) + 's' : p.time_window_min + 'm'}</td>
                        </tr>`;
                    });
                    html += '</table>';
                    pumpDiv.innerHTML = html;
                } else {
                    pumpDiv.innerHTML = '<div class="empty-state">No pumps detected yet. Monitoring (5% in 5s)...</div>';
                }
                ...
                if (data.open_trades.length > 0) {
                    ...
                    data.open_trades.forEach(t => {
                        ...
                        let asset_o = t.symbol.replace('USDT','').replace('USDC','');
                        html += `<tr>
                            <td>${t.entry_time_str}</td>
                            <td><a href="https://www.binance.com/en/trade/${asset_o}_USDT" target="_blank" style="color:#58a6ff;text-decoration:none;"><strong>${t.symbol}</strong></a></td>
                            <td><a href="https://www.tradingview.com/chart/?symbol=BINANCE:${t.symbol}" target="_blank" style="color:#58a6ff;text-decoration:underline;">${formatNumber(t.entry_price, 8)}</a></td>
                ...
                if (data.closed_trades.length > 0) {
                    ...
                    data.closed_trades.forEach(t => {
                        let pnlClass = t.pnl_percent >= 0 ? 'pnl-positive' : 'pnl-negative';
                        let asset_c = t.symbol.replace('USDT','').replace('USDC','');
                        html += `<tr>
                            <td>${t.entry_time_str}</td>
                            <td><a href="https://www.binance.com/en/trade/${asset_c}_USDT" target="_blank" style="color:#58a6ff;text-decoration:none;"><strong>${t.symbol}</strong></a></td>
                            <td><a href="https://www.tradingview.com/chart/?symbol=BINANCE:${t.symbol}" target="_blank" style="color:#58a6ff;text-decoration:underline;">${formatNumber(t.entry_price, 8)}</a></td>
        ...
        async function savePreset() {
            ...
            let preset = {
                ...
                max_trades_per_day: parseInt(document.getElementById('btMaxPerDay').value),
                pair_mode: document.getElementById('btPairMode').value
            };
        ...
        async function loadPreset() {
            ...
            try {
                ...
                document.getElementById('btMaxPerDay').value = p.max_trades_per_day || 0;
                document.getElementById('btPairMode').value = p.pair_mode || 'all';
            } catch(e) {}
        }
        ...
        async function refineWithTicks() {
            ...
            try {
                ...
                let timeoutId = setTimeout(() => controller.abort(), 1800000);
                let resp = await fetch('api/backtest/refine', {
                    ...
                    body: JSON.stringify({
                        ...
                        max_trades_per_day: lastPayload.max_trades_per_day,
                        pair_mode: lastPayload.pair_mode
                    })
                });
        ...
        async function saveSettings() {
            ...
            let payload = {
                ...
                max_trades_per_day: parseInt(document.getElementById('setMaxPerDay').value),
                pair_mode: document.getElementById('setPairMode').value
            };
...
def create_app(config: dict, database: Database, detector_ref: list) -> Flask:
    ...
    _usdc_bases_cache = {"ts": 0, "bases": set()}
    def _get_usdc_bases(rest_url_param):
        """Get set of base assets that have active USDC trading pairs."""
        import time as _tt
        if _tt.time() - _usdc_bases_cache["ts"] < 3600:
            return _usdc_bases_cache["bases"]
        try:
            import requests as _rq
            r = _rq.get(rest_url_param + "/api/v3/exchangeInfo", timeout=30)
            info = r.json()
            bases = set()
            for s in info.get("symbols", []):
                if s.get("quoteAsset") == "USDC" and s.get("status") == "TRADING":
                    bases.add(s["baseAsset"])
            _usdc_bases_cache["bases"] = bases
            _usdc_bases_cache["ts"] = _tt.time()
            return bases
        except Exception:
            return _usdc_bases_cache["bases"]

    def _filter_symbols_by_pair_mode(symbols, pair_mode, rest_url_param):
        """Filter symbols based on pair mode (usdc/convert/all)."""
        if pair_mode == "all":
            return symbols
        usdc_bases = _get_usdc_bases(rest_url_param)
        if not usdc_bases:
            return symbols
        if pair_mode == "usdc":
            return [s for s in symbols if s.replace("USDT", "") in usdc_bases]
        elif pair_mode == "convert":
            return [s for s in symbols if s.replace("USDT", "") not in usdc_bases]
        return symbols
    ...
    def index():
        ...
        return render_template_string(
            ...
            max_trades_per_day=trading_cfg.get("max_trades_per_day", 0),
            pair_mode=trading_cfg.get("pair_mode", "all")
        )
    ...
    def api_data():
        ...
        if detector_ref:
            ...
            stats["active_config"] = {
                ...
                "max_trades_per_day": getattr(detector, "max_trades_per_day", 0),
        ...
        response = jsonify({
            ...
            "pumps": database.get_recent_pumps(50),
    ...
    def history_scan():
        ...
        try:
            ...
            symbols = _filter_symbols_by_pair_mode(symbols, pair_mode, rest_url)
    ...
    def _run_tick_backtest(req, rest_url, quote, start_ts, end_ts,
                          ...
                          max_duration_hours=0, max_trades_per_day=0, pair_mode="all"):
        ...
        if max_trades_per_day > 0 and all_trades:
            daily_filtered = []
            day_counts = {}
            for t in all_trades:
                entry_day = t["entry_date"][:10]
                cnt = day_counts.get(entry_day, 0)
                if cnt >= max_trades_per_day:
                    continue
                day_counts[entry_day] = cnt + 1
                daily_filtered.append(t)
            all_trades = daily_filtered
    ...
    def _run_hybrid_backtest(req, rest_url, quote, start_ts, end_ts,
        ...
        for _retry in range(3):
            ...
            return jsonify({"error": "Failed to fetch ticker data from Binance"})
        usdt_pairs = [t for t in tickers if t["symbol"].endswith(quote) and float(t["quoteVolume"]) > 0]
        usdt_pairs.sort(key=lambda x: float(x["quoteVolume"]), reverse=True)
        symbols = [t["symbol"] for t in usdt_pairs[:top_n]]
        symbols = _filter_symbols_by_pair_mode(symbols, pair_mode, rest_url)
    ...
    def backtest_refine():
        ...
        try:
            ...
            max_trades_per_day = int(data.get("max_trades_per_day", 0))
            pair_mode = data.get("pair_mode", "all")

            if pair_mode != "all":
                rest_url_r = config.get("binance", {}).get("rest_url", "https://api.binance.com")
                input_trades = [t for t in input_trades if t["symbol"] in _filter_symbols_by_pair_mode([t["symbol"]], pair_mode, rest_url_r)]
            ...
            def refine_trade(te):
                try:
                    ...
                    if not got_any:
                        # Fallback: use 1-minute klines when aggTrades not available
                        try:
                            kline_start = entry_ts - window_sec * 1000
                            max_dur_ms_fb = int(max_duration_hours * 3600 * 1000) if max_duration_hours > 0 else 86400000
                            kline_end = entry_ts + max_dur_ms_fb
                            kr = req.get(f"{rest_url}/api/v3/klines", params={
                                "symbol": symbol, "interval": "1m",
                                "startTime": kline_start, "endTime": kline_end, "limit": 1000
                            }, timeout=10)
                            klines = kr.json()
                            if klines and isinstance(klines, list) and len(klines) > 1:
                                ...
                                for k in klines:
                                    ...
                                    k_close = float(k[4])
                                    k_close_ts = k[6] / 1000
                                    fb_window.append((k_open_ts, k_high))
                                    fb_window.append((k_open_ts, k_low))
                                    cutoff_fb = k_open_ts - window_sec
                                    while fb_window and fb_window[0][0] < cutoff_fb:
                                        fb_window.popleft()
                                    if fb_in_trade:
                                        pnl_high = ((k_high - fb_entry_price) / fb_entry_price) * 100
                                        pnl_low = ((k_low - fb_entry_price) / fb_entry_price) * 100
                                        if pnl_high >= tp:
                                            exit_p = fb_entry_price * (1 + tp / 100)
                                            return {
                                                "symbol": symbol,
                                                "entry_date": datetime.fromtimestamp(fb_entry_ts).strftime("%Y-%m-%d %H:%M:%S"),
                                                "exit_date": datetime.fromtimestamp(k_open_ts).strftime("%Y-%m-%d %H:%M:%S"),
                                                "entry_price": fb_entry_price, "exit_price": round(exit_p, 8),
                                                "pnl_pct": round(tp, 2), "pnl_usdt": round(amount * tp / 100, 2),
                                                "exit_reason": "TP", "entry_type": entry_type
                                            }
                                        if pnl_low <= -sl:
                                            exit_p = fb_entry_price * (1 - sl / 100)
                                            return {
                                                "symbol": symbol,
                                                "entry_date": datetime.fromtimestamp(fb_entry_ts).strftime("%Y-%m-%d %H:%M:%S"),
                                                "exit_date": datetime.fromtimestamp(k_open_ts).strftime("%Y-%m-%d %H:%M:%S"),
                                                "entry_price": fb_entry_price, "exit_price": round(exit_p, 8),
                                                "pnl_pct": round(-sl, 2), "pnl_usdt": round(-amount * sl / 100, 2),
                                                "exit_reason": "SL", "entry_type": entry_type
                                            }
                                        if max_duration_hours > 0 and (k_open_ts - fb_entry_ts) >= max_duration_hours * 3600:
                                            pnl_final = ((k_close - fb_entry_price) / fb_entry_price) * 100
                                            return {
                                                "symbol": symbol,
                                                "entry_date": datetime.fromtimestamp(fb_entry_ts).strftime("%Y-%m-%d %H:%M:%S"),
                                                "exit_date": datetime.fromtimestamp(k_open_ts).strftime("%Y-%m-%d %H:%M:%S"),
                                                "entry_price": fb_entry_price, "exit_price": round(k_close, 8),
                                                "pnl_pct": round(pnl_final, 2), "pnl_usdt": round(amount * pnl_final / 100, 2),
                                                "exit_reason": "TIME", "entry_type": entry_type
                                            }
                                    else:
                                        if k[0] < entry_ts:
                                            continue
                                        max_p = max((p for _, p in fb_window), default=0)
                                        if max_p > 0:
                                            drop_fb = ((max_p - k_low) / max_p) * 100
                                            if drop_fb >= crash_threshold:
                                                fb_in_trade = True
                                                fb_entry_price = k_low
                                                fb_entry_ts = k_open_ts
                        except Exception:
                            pass
                        return te
        ...
        try:
            ...
            if bt_mode == "tick":
                ...
                return _run_tick_backtest(
                    ...
                    max_duration_hours, max_trades_per_day, pair_mode
                )
    ...
    def save_settings():
        ...
        try:
            ...
            if "max_trades_per_day" in data:
                ...
                trading_cfg["max_trades_per_day"] = val
                if detector_ref:
                    detector_ref[0].max_trades_per_day = val

            if "pair_mode" in data:
                val = str(data["pair_mode"])
                trading_cfg["pair_mode"] = val
                if detector_ref:
                    detector_ref[0].pair_mode = val
