...
class Database:
    ...
    def record_crash(self, symbol, price_start, price_end, drop_percent,
                     time_window_min, window_seconds, signal_type="CRASH"):
        """Record a detected crash or pump event. Returns the crash ID."""
        now = time.time()
        ...
        with self._lock:
            cursor = conn.execute("""
                ...
                (symbol, detected_at, detected_at_str, price_start, price_end,
                 drop_percent, time_window_min, window_seconds, signal_type)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    def record_trade(self, crash_id, symbol, entry_price, quantity, amount_eur,
                     is_live=False, order_id=None, tp_pct=None, sl_pct=None, max_dur_hours=None, trade_mode=None):
        ...
        with self._lock:
            cursor = conn.execute("""
                ...
                (crash_id, symbol, entry_price, current_price, quantity,
                 amount_eur, entry_time, entry_time_str, status, is_live, order_id, tp_pct, sl_pct, max_dur_hours, trade_mode)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'OPEN', ?, ?, ?, ?, ?, ?)
            """, (crash_id, symbol, entry_price, entry_price, quantity,
                  amount_eur, now, now_str, 1 if is_live else 0, order_id, tp_pct, sl_pct, max_dur_hours, trade_mode))
            conn.commit()
            return cursor.lastrowid
    ...
    def get_recent_crashes(self, limit=100):
        """Get recent crash events (excludes pumps)."""
        conn = self._get_conn()
        rows = conn.execute(
            "SELECT * FROM crashes WHERE signal_type IS NULL OR signal_type = 'CRASH' ORDER BY detected_at DESC LIMIT ?",
            (limit,)
        ).fetchall()
        return [dict(r) for r in rows]

    def get_recent_pumps(self, limit=100):
        """Get recent pump events."""
        conn = self._get_conn()
        rows = conn.execute(
            "SELECT * FROM crashes WHERE signal_type = 'PUMP' ORDER BY detected_at DESC LIMIT ?",
            (limit,)
        ).fetchall()
        return [dict(r) for r in rows]
