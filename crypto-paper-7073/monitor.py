        ...
        if self.live_mode and self.trader and self.trader.live_mode:
            ...
            if result:
                ...
                logger.info(
                    ...
                    f"@ {result['avg_price']:.8g} for {result['filled_quote']:.2f} USDC "
                    f"[Trade #{trade_id}]"
                )
    ...
    def _forward_to_1111(self, symbol, price):
        try:
            ...
            resp = _req.post(
                ...
                json={"symbol": symbol, "volume_ratio": 3.0, "ob_ratio": 1.5, "price": price, "source_port": 7073},
...
class BinanceMonitor:
    ...
    def _fetch_top_pairs_binance(self) -> List[str]:
        ...
        try:
            ...
            # Filter USDC pairs and sort by quote volume
            usdt_pairs = [
                t for t in tickers
