...
<head>
    ...
    <title>Scalping Pump (9002 - Cycle Filtered)</title>
    <style>
...
<body>
    <div class="header">
        <h1>Scalping Pump <span style="font-size:14px;color:#f0883e;font-weight:400">(9002 - Cycle Filtered)</span></h1>
        <div class="status">
    ...
    <div class="container">
        ...
        <div style="display:flex; flex-wrap:wrap; gap:8px; align-items:center; margin-top:8px; padding:10px 16px; background:#161b22; border:1px solid #30363d; border-radius:6px;">
            ...
            <div style="display:flex;align-items:center;gap:4px;">
                <label style="color:#3fb950;font-size:12px;white-space:nowrap;">TP Book USDT &ge;</label>
            ...
            <div style="display:flex;align-items:center;gap:4px;">
                <label style="color:#f85149;font-size:12px;white-space:nowrap;">SL Book USDT &le; -</label>
...
_binance_cache_9002 = {"portfolio_ext": None, "ts": 0, "open_books": {}}
_proxy_cache = {"usdc_balance": 0, "binance_total_eur": 0, "binance_holdings": [], "ts": 0}
_COND_FILE = "/opt/crypto-paper-9002/cond_close.json"
...
def _save_cond_state():
    try:
        with open(_COND_FILE, "w") as f:
            _json_cond.dump(_cond_state_9002, f)
    except Exception:
        pass
_cond_state_9002 = _load_cond_state()
...
def create_app(config: dict, database: Database, detector_ref: list) -> Flask:
    ...
    def api_data():
        ...
        global _binance_cache_9002
        _now = _time.time()
        _cache_ttl = 60
        if _now - _binance_cache_9002["ts"] > _cache_ttl:
            ...
            if open_trades:
                ...
                for t in open_trades:
                    ...
                    if _bp and _bp.get("bid", 0) > 0:
                        ...
                        _new_books[t["symbol"]] = _binance_cache_9002.get("open_books", {}).get(t["symbol"], cp)
            ...
            _binance_cache_9002["portfolio_ext"] = _pext
            _binance_cache_9002["open_books"] = _new_books
            _binance_cache_9002["ts"] = _now

        portfolio_ext = _binance_cache_9002.get("portfolio_ext") or {"open_count": len(open_trades), "total_invested": 0, "current_value": 0, "total_pnl": 0, "pnl_book": 0}
        portfolio_ext["open_count"] = len(open_trades)
        for t in open_trades:
            t["book_price"] = _binance_cache_9002.get("open_books", {}).get(t["symbol"], t.get("current_price", t["entry_price"]))
        ...
        try:
            ...
            _today_pnl = sum(t.get("pnl_eur", 0) for t in closed_trades if (t.get("exit_time_str") or "").startswith(_today_str))
            _pct = round(_today_pnl / _total_usdc * 100, 2) if _total_usdc > 0 else 0
            resp_data["pnl_today"] = {"usdc": round(_today_pnl, 2), "pct": str(_pct)}
        except Exception:
            resp_data["pnl_today"] = {"usdc": 0, "pct": "0"}

        # Server-side conditional close check
        global _cond_state_9002
        if _cond_state_9002["active"] and resp_data.get("portfolio_ext"):
            _pe = resp_data["portfolio_ext"]
            _book = _pe.get("pnl_book", 0)
            _oc = _pe.get("open_count", 0)
            if _cond_state_9002["cooldown"]:
                if _oc > 0:
                    _cond_state_9002["cooldown"] = False
                    _cond_state_9002["status"] = f"Watching. TP>={_cond_state_9002['tp']:.2f}" if _cond_state_9002["tp"] > 0 else "Watching."
                    if _cond_state_9002["sl"] > 0:
                        _cond_state_9002["status"] += f" SL<=-{_cond_state_9002['sl']:.2f}"
                    _cond_state_9002["status_color"] = "#f0883e"
            elif _oc > 0:
                _triggered = False
                _reason = ""
                if _cond_state_9002["tp"] > 0 and _book >= _cond_state_9002["tp"]:
                    _triggered = True
                    _reason = f"TP TRIGGERED! Book={_book:.2f} USDT."
                    _cond_state_9002["status_color"] = "#3fb950"
                if _cond_state_9002["sl"] > 0 and _book <= -_cond_state_9002["sl"]:
                    _triggered = True
                    _reason = f"SL TRIGGERED! Book={_book:.2f} USDT."
                    _cond_state_9002["status_color"] = "#f85149"
                if _triggered:
                    try:
                        ...
                        pass
                    if _cond_state_9002["mode"] == "once":
                        _cond_state_9002["active"] = False
                        _cond_state_9002["mode"] = "off"
                        _cond_state_9002["cooldown"] = False
                        _cond_state_9002["status"] = _reason + " Closed. (One time - deactivated)"
                        _save_cond_state()
                    else:
                        _cond_state_9002["cooldown"] = True
                        if _cond_state_9002["mode"] == "random" and _cond_state_9002["tp_base"] > 0:
                            _pct = 1 + (_random.random() * 9 + 1) / 100
                            _cond_state_9002["tp"] = round(_cond_state_9002["tp_base"] * _pct, 4)
                            _cond_state_9002["status"] = _reason + f" Closed. Next TP={_cond_state_9002['tp']:.4f}. Waiting..."
                            _save_cond_state()
                        else:
                            _cond_state_9002["status"] = _reason + " Closed. Waiting for next round..."
                            _save_cond_state()

        resp_data["_v"] = 2
        resp_data["cond_state"] = {
            "active": _cond_state_9002["active"],
            "mode": _cond_state_9002["mode"],
            "tp": _cond_state_9002["tp"],
            "tp_base": _cond_state_9002["tp_base"],
            "sl": _cond_state_9002["sl"],
            "cooldown": _cond_state_9002["cooldown"],
            "status": _cond_state_9002["status"],
            "status_color": _cond_state_9002["status_color"]
        }
    ...
    def get_cond_state():
        global _cond_state_9002
        return jsonify(_cond_state_9002)
    ...
    def set_cond_state():
        ...
        if data.get("action") == "activate":
            _cond_state_9002["active"] = True
            _cond_state_9002["mode"] = data.get("mode", "once")
            _cond_state_9002["tp"] = float(data.get("tp", 0))
            _cond_state_9002["tp_base"] = float(data.get("tp", 0))
            _cond_state_9002["sl"] = float(data.get("sl", 0))
            _cond_state_9002["cooldown"] = False
            if _cond_state_9002["mode"] == "random" and _cond_state_9002["tp_base"] > 0:
                _pct = 1 + (_random.random() * 9 + 1) / 100
                _cond_state_9002["tp"] = round(_cond_state_9002["tp_base"] * _pct, 4)
            label = {"once": "ONE TIME", "always": "ALWAYS", "random": "ALWAYS +RANDOM"}.get(_cond_state_9002["mode"], _cond_state_9002["mode"])
            msg = f"{label}: "
            if _cond_state_9002["tp"] > 0:
                msg += f"TP >= {_cond_state_9002['tp']:.2f} USDT"
                if _cond_state_9002["mode"] == "random":
                    msg += f" (base {_cond_state_9002['tp_base']:.2f} +1-10%)"
                msg += " "
            if _cond_state_9002["sl"] > 0:
                msg += f"SL <= -{_cond_state_9002['sl']:.2f} USDT"
            _cond_state_9002["status"] = msg
            _cond_state_9002["status_color"] = "#f0883e"
            _save_cond_state()
            return jsonify({"success": True, "state": _cond_state_9002})
        elif data.get("action") == "deactivate":
            _cond_state_9002 = {"active": False, "mode": "off", "tp": 0, "tp_base": 0, "sl": 0, "cooldown": False, "status": "Deactivated", "status_color": "#8b949e"}
            return jsonify({"success": True, "state": _cond_state_9002})
