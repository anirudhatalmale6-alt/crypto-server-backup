...
def _cond_monitor_loop():
    while True:
        ...
        try:
            ...
            current_eur = 0
            try:
                with open("/opt/crypto-shared/cache.json", "r") as f:
                    _cc = json.load(f)
                _bals = _cc.get("balances", {})
                _tps = _cc.get("ticker_prices", {})
                _eur_r = _cc.get("eur_rate", 1.13)
                _tot = 0
                for _a, _q in _bals.items():
                    if _q < 0.0001: continue
                    if _a == "USDT": _v = _q
                    elif _a == "USDC": _v = _q * _tps.get("USDCUSDT", 1.0)
                    elif _a == "EUR": _v = _q * _eur_r
                    elif _a == "BTC": _v = _q * _tps.get("BTCUSDT", 0)
                    elif _a == "ETH": _v = _q * _tps.get("ETHUSDT", 0)
                    else: _v = _q * _tps.get(f"{_a}USDT", _tps.get(f"{_a}USDC", 0))
                    _tot += _v / _eur_r if _eur_r > 0 else 0
                current_eur = round(_tot, 2)
            except Exception:
                pass
            if current_eur < 100:
                continue
...
# --- Per-port conditioned monitoring ---
_pcond_file = "/opt/crypto-paper-9999/port_conditioned_settings.json"
_pcond_lock = threading.Lock()
_pcond_log = logging.getLogger("port_conditioned")

def _pcond_load():
    try:
        with open(_pcond_file, "r") as f:
            return json.load(f)
    except Exception:
        return {}

def _pcond_save(data):
    with _pcond_lock:
        with open(_pcond_file, "w") as f:
            json.dump(data, f)

def _pcond_monitor_loop():
    while True:
        _time.sleep(5)
        try:
            settings = _pcond_load()
            if not settings:
                continue
            for port_str, cfg in list(settings.items()):
                if not cfg.get("active"):
                    continue
                port = int(port_str)
                tp_abs = cfg.get("tp_abs", 0)
                sl_abs = cfg.get("sl_abs", 0)
                if tp_abs <= 0 and sl_abs <= 0:
                    continue
                try:
                    ...
                    if tp_abs > 0 and total_pnl >= tp_abs:
                        triggered = True
                        reason = f"PORT {port} TP: P/L {total_pnl} >= {tp_abs}"
                    elif sl_abs > 0 and total_pnl <= -sl_abs:
                        triggered = True
                        reason = f"PORT {port} SL: P/L {total_pnl} <= -{sl_abs}"
                    duration = cfg.get("duration", "forever")
                    expires_at = cfg.get("expires_at")
                    if expires_at and _time.time() > expires_at:
                        _pcond_log.info(f"PORT {port} CONDITIONED EXPIRED after {cfg.get('hours',0)} hours")
                        settings = _pcond_load()
                        if str(port) in settings:
                            del settings[str(port)]
                            _pcond_save(settings)
                        continue
                    if triggered:
                        _pcond_log.warning(f"PORT CONDITIONED TRIGGERED: {reason}")
                        settings = _pcond_load()
                        if str(port) in settings:
                            del settings[str(port)]
                            _pcond_save(settings)
                        try:
                            ...
                            cd = cr.json()
                            _pcond_log.warning(f"PORT {port} CLOSED: {cd.get('closed', 0)} positions")
                        except Exception as e:
                            _pcond_log.error(f"PORT {port} CLOSE ERROR: {e}")

                except Exception as e:
                    _pcond_log.error(f"PORT {port} CHECK ERROR: {e}")
        except Exception as e:
            _pcond_log.error(f"PORT CONDITIONED MONITOR ERROR: {e}")

_pcond_thread = threading.Thread(target=_pcond_monitor_loop, daemon=True)
_pcond_thread.start()


PORTS = [7081, 5065, 5057, 8010, 8011, 8012, 8082, 8083, 8084, 7071, 7072, 8090, 8091, 8092, 8093, 8094, 8095, 8096, 8097, 8098, 8099, 9000, 9001, 9002, 9003, 9010, 9011, 9012]
...
def get_actual_pnl(port):
    ...
    try:
        ...
        d = r.json()
        if port == 5065:
            val = round(d.get("total_pnl", 0) or 0, 2)
            inv = round(d.get("total_invested", 0) or 0, 2)
        else:
            p = d.get("portfolio", {})
...
def query_port(port, start_date=None):
    if port == 5065:
        try:
            ...
            "open_last_60m": open_last_60m,
                ...
                "port_cond": _pcond_load().get("5065", {}),
    ...
    try:
        ...
        tw = cfg.get("detection", {}).get("time_windows_minutes", [])
        window = ", ".join(str(w) + "m" for w in tw) if tw else str(cfg.get("detection", {}).get("pump_window_seconds", 0)) + "s"
        ...
        return {
            ...
            "invested": _actual_inv_cache.get(port, 0),
                "port_cond": _pcond_load().get(str(port), {}),
            "skip_rate_limit": skips["skip_rate_limit"],
            "skip_no_pair": skips["skip_no_pair"],
            "skip_max_trades": skips["skip_max_trades"]
        }
        ...
        return {
            ...
            "mode": cfg.get("trading", {}).get("mode", "demo"),
            "amount": cfg.get("trading", {}).get("amount_per_trade_eur", 0),
            "window": ", ".join(str(w) + "m" for w in cfg.get("detection", {}).get("time_windows_minutes", [])) if cfg.get("detection", {}).get("time_windows_minutes", []) else str(cfg.get("detection", {}).get("pump_window_seconds", 0)) + "s",
            "total_pnl": 0, "total_closed": 0, "open_count": 0, "open_live": 0, "open_1111": 0, "open_demo": 0, "open_last_60m": 0,
            "max_open_history": 0, "wins": 0, "losses": 0,
...
_close_jobs = {}
def _run_close_all(port):
    import threading
    try:
        r = req.post(f"http://127.0.0.1:{port}/api/close-all", timeout=300)
        _close_jobs[port] = {"status": "done", "result": r.json()}
    except Exception as e:
        _close_jobs[port] = {"status": "error", "result": {"error": str(e)}}
...
def api_portfolio():
    ...
    try:
        ...
        ticker_prices = cache.get("ticker_prices", {})
        eur_usdt = cache.get("eur_rate", 1.13)
        ...
        for asset, qty in balances.items():
            ...
            if asset == "USDT":
                ...
                val_usdt = qty * ticker_prices.get("USDCUSDT", 1.0)
                usdc_balance = round(qty, 2)
            elif asset == "EUR":
                val_usdt = qty * eur_usdt
            elif asset == "BTC":
                val_usdt = qty * ticker_prices.get("BTCUSDT", 0)
            elif asset == "ETH":
                val_usdt = qty * ticker_prices.get("ETHUSDT", 0)
            else:
                val_usdt = qty * ticker_prices.get(f"{asset}USDT", ticker_prices.get(f"{asset}USDC", 0))
            val_eur = val_usdt / eur_usdt if eur_usdt > 0 else 0
            if val_eur > 0.01:
                binance_holdings.append({"asset": asset, "qty": round(qty, 4), "eur": round(val_eur, 2)})
                binance_total_eur += val_eur
                total_usdt += val_usdt

        binance_holdings.sort(key=lambda x: x["eur"], reverse=True)
        binance_total_eur = round(binance_total_eur, 2)

        eur_free = round(next((h["eur"] for h in binance_holdings if h["asset"] == "EUR"), 0), 2)
        eur_free_usdt = eur_free * eur_usdt

        try:
            import sqlite3 as _sq3
            _db7081 = _sq3.connect("file:///opt/crypto-paper-7081/paper_7081.db?mode=ro", uri=True)
            _c7081 = _db7081.cursor()
            _c7081.execute("SELECT symbol, quantity FROM trades WHERE exit_time IS NULL AND is_live=1")
            _p7081_live = _c7081.fetchall()
            eur_invested = round(sum(_qty7 * ticker_prices.get(_sym7, 0) / eur_usdt for _sym7, _qty7 in _p7081_live), 2)
            _db7081.close()
        except Exception:
            eur_invested = 0

        eur_invested_usdt = eur_invested * eur_usdt
        usdc_portfolio = round(total_usdt - eur_free_usdt - eur_invested_usdt, 2)
        usdc_inv = round(usdc_portfolio - usdc_balance, 2) if usdc_portfolio > 0 else 0
        usdc_avl = round(usdc_balance, 2)
    except Exception as e:
        app.logger.error(f"Portfolio cache read error: {e}")

    port_data = {}
    try:
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = {executor.submit(_fetch_port_data, p): p for p in LIVE_PORTS}
    ...
    total_portfolio = usdc_balance + total_invested + total_unrealized
...
def api_port_close_all(port):
    import threading
    _close_jobs[port] = {"status": "processing", "result": None}
    t = threading.Thread(target=_run_close_all, args=(port,), daemon=True)
    t.start()
    return jsonify({"status": "processing", "port": port})

@app.route("/api/port-close-result/<int:port>")
def api_port_close_result(port):
    job = _close_jobs.get(port)
    if not job:
        return jsonify({"status": "none"})
    if job["status"] == "processing":
        return jsonify({"status": "processing"})
    result = job["result"]
    del _close_jobs[port]
    return jsonify({"status": job["status"], "result": result})
...
def api_port_close_conditioned(port):
    ...
    try:
        ...
        if port == 5065:
            positions = d.get("positions", [])
            if not positions:
                return jsonify({"success": False, "error": "No open positions"})
            total_pnl = round(d.get("total_pnl", 0), 2)
        else:
            trades = d.get("open_trades", [])
            if not trades:
...
@app.route("/api/port-conditioned-save/<int:port>", methods=["POST"])
def api_port_conditioned_save(port):
    try:
        data = request.get_json(force=True) or {}
        tp_abs = float(data.get("tp_abs", 0))
        sl_abs = float(data.get("sl_abs", 0))
        ...
        if tp_abs <= 0 and sl_abs <= 0:
            return jsonify({"error": "Set TP or SL first"}), 400
        expires_at = None
        if duration == "hours" and hours > 0:
            expires_at = _time.time() + hours * 3600
        settings = _pcond_load()
        settings[str(port)] = {"active": True, "tp_abs": tp_abs, "sl_abs": sl_abs, "duration": duration, "hours": hours, "expires_at": expires_at, "saved_at": _time.time()}
        _pcond_save(settings)
        return jsonify({"success": True, "port": port, "tp_abs": tp_abs, "sl_abs": sl_abs, "duration": duration})
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route("/api/port-conditioned-status/<int:port>")
def api_port_conditioned_status(port):
    settings = _pcond_load()
    cfg = settings.get(str(port), {"active": False, "tp_abs": 0, "sl_abs": 0})
    return jsonify(cfg)

@app.route("/api/port-conditioned-clear/<int:port>", methods=["POST"])
def api_port_conditioned_clear(port):
    settings = _pcond_load()
    if str(port) in settings:
        del settings[str(port)]
        _pcond_save(settings)
...
function render(data) {
    ...
    ports.forEach(p => {
        ...
        var condBadge = '';
        if (pcond.active) { var dl = pcond.duration === 'onetime' ? ' 1x' : pcond.duration === 'hours' ? ' ' + (pcond.hours||0) + 'h' : ''; condBadge = '<div style="font-size:8px;color:#3fb950;margin-top:2px;font-weight:700">COND' + dl + ': TP +' + (pcond.tp_abs||0) + ' / SL -' + (pcond.sl_abs||0) + '</div>'; }
        h += '<td style="text-align:center"><span onclick="openPnlModal(' + p.port + ',' + apnl + ',' + apInv + ')" style="cursor:pointer;display:inline-block;background:' + apBg + ';color:' + apTx + ';padding:2px 8px;border-radius:4px;font-weight:700;font-size:11px">' + (apnl >= 0 ? '+' : '') + fmt(apnl) + ' <span style="font-weight:400;font-size:9px;opacity:0.85">(' + apPct + '%)</span></span>' + condBadge + '</td>';
...
function openPnlModal(port, pnl, invested) {
    ...
    fetch('/api/port-config/' + port + '?t=' + Date.now())
        ...
        .then(function(d) {
            ...
            h += '</div></div>';
            h += '</div>';
            h += '<div style="display:flex;gap:12px;margin-top:14px;align-items:center;justify-content:center">';
            h += '<label style="color:#c9d1d9;font-size:12px;cursor:pointer"><input type="radio" name="condDuration" value="onetime" checked style="margin-right:4px">ONE TIME</label>';
            h += '<label style="color:#c9d1d9;font-size:12px;cursor:pointer"><input type="radio" name="condDuration" value="hours" style="margin-right:4px"><input id="condHoursVal" type="number" min="1" max="999" value="2" style="width:40px;background:#161b22;color:#f0883e;border:1px solid #30363d;padding:2px 4px;border-radius:4px;font-size:12px;font-weight:700;text-align:center"> HOURS</label>';
            h += '<label style="color:#c9d1d9;font-size:12px;cursor:pointer"><input type="radio" name="condDuration" value="forever" style="margin-right:4px">FOREVER</label>';
            h += '</div>';
            h += '<div style="display:flex;gap:10px;margin-top:16px">';
            h += '<button id="portCondApplyBtn" onclick="portCondApply(' + port + ')" style="background:#1f6feb;color:#fff;border:2px solid #58a6ff;border-radius:6px;padding:10px 20px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit;flex:1">APPLY</button>';
            h += '<button id="portCondSaveBtn" onclick="portCondSave(' + port + ')" style="background:#238636;color:#fff;border:2px solid #3fb950;border-radius:6px;padding:10px 20px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit;flex:1">SAVE</button>';
            ...
            fetch('/api/port-conditioned-status/' + port).then(function(r){return r.json();}).then(function(cs) {
                if (cs.active) {
                    var dur = cs.duration || 'forever';
                    var durLabel = dur === 'onetime' ? 'ONE TIME' : dur === 'hours' ? (cs.hours||0) + ' HOURS' : 'FOREVER';
                    var expiry = '';
                    if (cs.expires_at) {
                        var left = Math.max(0, Math.round((cs.expires_at - Date.now()/1000) / 60));
                        if (left > 0) expiry = ' (' + left + ' min left)';
                    }
                    document.getElementById('pnlStatus').innerHTML = 'Server monitoring ACTIVE - ' + durLabel + expiry + ' (TP: +' + (cs.tp_abs||0) + ' / SL: -' + (cs.sl_abs||0) + ') <button onclick="portCondCancel(' + port + ')" style="background:#b91c1c;color:#fff;border:1px solid #f85149;border-radius:4px;padding:3px 12px;font-size:11px;font-weight:700;cursor:pointer;margin-left:10px;font-family:inherit">CANCEL</button>';
                    document.getElementById('pnlStatus').style.color = '#3fb950';
                    if (cs.tp_abs > 0) { document.getElementById('pnlTpAbs').value = cs.tp_abs; tpAbsChange(); }
                    if (cs.sl_abs > 0) { document.getElementById('pnlSlAbs').value = cs.sl_abs; slAbsChange(); }
                    var radios = document.querySelectorAll('input[name="condDuration"]');
                    radios.forEach(function(r) { r.checked = (r.value === dur); });
                    if (dur === 'hours' && cs.hours) { var hv = document.getElementById('condHoursVal'); if (hv) hv.value = cs.hours; }
                }
            }).catch(function(){});
        })
...
function closePnlModal() { if (_portCondMonitor) { clearInterval(_portCondMonitor); _portCondMonitor = null; } document.getElementById('pnlModal').classList.remove('open'); }
var _portCondMonitor = null;
var _portCondPort = null;
function portCondApply(port) {
    ...
    var dur = 'forever';
    var radios = document.querySelectorAll('input[name="condDuration"]');
    radios.forEach(function(r) { if (r.checked) dur = r.value; });
    _portCondPort = port;
    document.getElementById('portCondApplyBtn').textContent = dur === 'onetime' ? 'CHECKING...' : 'MONITORING...';
    document.getElementById('portCondApplyBtn').disabled = true;
    document.getElementById('portCondApplyBtn').style.opacity = '0.5';
    document.getElementById('pnlTpPct').disabled = true;
    document.getElementById('pnlTpAbs').disabled = true;
    document.getElementById('pnlSlPct').disabled = true;
    document.getElementById('pnlSlAbs').disabled = true;
    if (dur === 'onetime') {
        document.getElementById('pnlStatus').textContent = 'Checking condition once... TP: +' + tpAbs + ' / SL: -' + slAbs;
        document.getElementById('pnlStatus').style.color = '#f0883e';
        _portCondCheckOnce(port, tpAbs, slAbs);
    } else {
        document.getElementById('pnlStatus').textContent = 'Monitoring P/L... checking every 3s. TP: +' + tpAbs + ' / SL: -' + slAbs;
        document.getElementById('pnlStatus').style.color = '#f0883e';
        _portCondCheckOnce(port, tpAbs, slAbs);
        _portCondMonitor = setInterval(function() { _portCondCheckOnce(port, tpAbs, slAbs); }, 3000);
    }
}
function _portCondCheckOnce(port, tpAbs, slAbs) {
    ...
    .then(function(d) {
        if (d.error) {
            ...
            if (_portCondMonitor) { clearInterval(_portCondMonitor); _portCondMonitor = null; }
            ...
            document.getElementById('pnlStatus').innerHTML = 'Monitoring... P/L: <span style="color:' + pnlColor + '">' + (pnl >= 0 ? '+' : '') + pnl.toFixed(2) + '</span> USDC | TP: +' + tpAbs + ' / SL: -' + slAbs;
            document.getElementById('pnlStatus').style.color = '#f0883e';
        }
    })
    .catch(function(e) {
        document.getElementById('pnlStatus').textContent = 'Error: ' + e.message + ' (retrying...)';
        document.getElementById('pnlStatus').style.color = '#f85149';
    });
}
function portCondCancel(port) {
    fetch('/api/port-conditioned-clear/' + port, { method: 'POST' })
    .then(function(r) { return r.json(); })
    .then(function(d) {
        if (d.success) {
            document.getElementById('pnlStatus').textContent = 'Monitoring cancelled';
            document.getElementById('pnlStatus').style.color = '#8b949e';
            setTimeout(function() { document.getElementById('pnlStatus').textContent = ''; }, 1500);
...
function portCondSave(port) {
    ...
    var radios = document.querySelectorAll('input[name="condDuration"]');
    radios.forEach(function(r) { if (r.checked) dur = r.value; });
    if (dur === 'hours') { hours = parseInt(document.getElementById('condHoursVal').value) || 2; }
    fetch('/api/port-conditioned-save/' + port, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({tp_abs: tpAbs, sl_abs: slAbs, duration: dur, hours: hours})
    })
    .then(function(r) { return r.json(); })
    .then(function(d) {
        if (d.success) {
            if (_portCondMonitor) { clearInterval(_portCondMonitor); _portCondMonitor = null; }
            var durLabel2 = dur === 'onetime' ? 'ONE TIME' : dur === 'hours' ? hours + ' HOURS' : 'FOREVER';
            document.getElementById('pnlStatus').textContent = 'Saved! ' + durLabel2 + ' monitoring port ' + port + ' (TP: +' + tpAbs + ' / SL: -' + slAbs + ')';
            document.getElementById('pnlStatus').style.color = '#3fb950';
            setTimeout(function() { closePnlModal(); }, 1500);
        } else {
            document.getElementById('pnlStatus').textContent = 'Error: ' + (d.error || 'unknown');
            document.getElementById('pnlStatus').style.color = '#f85149';
        }
    })
    .catch(function(e) {
    ...
    fetch('/api/port-close-all/' + port, { method: 'POST' })
        ...
        .then(function(d) {
            if (d.status === 'processing') {
                document.getElementById('pnlStatus').textContent = 'Selling positions on Binance...';
                pollCloseResult(port);
            } else if (d.error) {
                document.getElementById('pnlStatus').textContent = 'Error: ' + d.error;
                document.getElementById('pnlStatus').style.color = '#f85149';
            } else {
...
function pollCloseResult(port) {
    fetch('/api/port-close-result/' + port)
        .then(function(r) { return r.json(); })
        .then(function(d) {
            if (d.status === 'processing') {
                setTimeout(function() { pollCloseResult(port); }, 2000);
            } else if (d.status === 'done') {
                document.getElementById('pnlStatus').textContent = '';
                showCloseAllResult(port, d.result);
                setTimeout(function() { refresh(); refreshPortfolio(); }, 2000);
            } else if (d.status === 'error') {
                document.getElementById('pnlStatus').textContent = 'Error: ' + (d.result && d.result.error || 'Unknown');
                document.getElementById('pnlStatus').style.color = '#f85149';
            }
        })
        .catch(function() {
            setTimeout(function() { pollCloseResult(port); }, 3000);
        });
}
